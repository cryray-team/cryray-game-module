...
float3 SSFX_DOF(float2 tc, float3 depth, float3 img)
{
	...
	if (blur_w > 0.f)
	{
		...
		img = lerp(img, blur_far, saturate(0.4f - (1.f - edgeBlur)));
	}
	
	// Far blur ( Reload, Inventory and PDA )
	if (ssfx_wpn_dof_1.z > 0.f)
	{
		// Let's use s_blur_2 for far blur
		img = lerp(img, blur_far, saturate(smoothstep(1.f, 2.4f, length(depth) ) + int(depth.z <= SKY_EPS)) * ssfx_wpn_dof_1.z);
	}
