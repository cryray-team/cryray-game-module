...
void ComputePNPatch(float3 P[3], float3 N[3], out PNPatch patch)
{
    ...
    // Edge control points
    patch.f3B210 = (2.f*P[0].xyz + P[1].xyz - dot(P[1].xyz-P[0].xyz, N[0])*N[0]) / 3.f;
    patch.f3B120 = (2.f*P[1].xyz + P[0].xyz - dot(P[0].xyz-P[1].xyz, N[1])*N[1]) / 3.f;
    patch.f3B021 = (2.f*P[1].xyz + P[2].xyz - dot(P[2].xyz-P[1].xyz, N[1])*N[1]) / 3.f;
    patch.f3B012 = (2.f*P[2].xyz + P[1].xyz - dot(P[1].xyz-P[2].xyz, N[2])*N[2]) / 3.f;
    patch.f3B102 = (2.f*P[2].xyz + P[0].xyz - dot(P[0].xyz-P[2].xyz, N[2])*N[2]) / 3.f;
    patch.f3B201 = (2.f*P[0].xyz + P[2].xyz - dot(P[2].xyz-P[0].xyz, N[0])*N[0]) / 3.f;
    
	// Center control point
    float3 f3E = ( patch.f3B210 + patch.f3B120 + patch.f3B021 + patch.f3B012 + patch.f3B102 + patch.f3B201 ) / 6.f;
    float3 f3V = ( P[0].xyz + P[1].xyz + P[2].xyz ) / 3.f;
    patch.f3B111 = f3E + ( ( f3E - f3V ) / 2.f );
    
    // Compute the quadratic normal control points, and rotate into world space
    float fV12 = 2.f * dot( P[1].xyz - P[0].xyz, N[0] + N[1] ) / dot( P[1].xyz - P[0].xyz, P[1].xyz - P[0].xyz );
    patch.f3N110 = normalize( N[0] + N[1] - fV12 * ( P[1].xyz - P[0].xyz ));
    float fV23 = 2.f * dot( P[2].xyz - P[1].xyz, N[1] + N[2] ) / dot( P[2].xyz - P[1].xyz, P[2].xyz - P[1].xyz );
    patch.f3N011 = normalize( N[1] + N[2] - fV23 * ( P[2].xyz - P[1].xyz ));
    float fV31 = 2.f * dot( P[0].xyz - P[2].xyz, N[2] + N[0] ) / dot( P[0].xyz - P[2].xyz, P[0].xyz - P[2].xyz );
...
void ComputePatchVertex(float3 P[3], float3 N[3], float3 uvw, in PNPatch patch, out float3 Pos, out float3 Norm)
{
	...
	Pos =	P[0] * w * w * w +
			P[1] * u * u * u +
			P[2] * v * v * v +
			patch.f3B210 * 3.f * w * w * u +
			patch.f3B120 * 3.f * w * u * u +
			patch.f3B201 * 3.f * w * w * v +
			patch.f3B021 * 3.f * u * u * v +
			patch.f3B102 * 3.f * w * v * v +
			patch.f3B012 * 3.f * u * v * v +
			patch.f3B111 * 6.f * w * u * v;
...
void ComputeDisplacedVertex(inout float3 P, float3 N, float2 tc, float2 tcd)
{
	...
	float3	Ne = Nu.wzy + (NuE.xyz - 1.f);	//(Nu.wzyx - .5h) + (E-.5)
	float	height = NuE.w;

#	ifdef  USE_TDETAIL_BUMP
	float4 NDetailX		= s_tdetailBumpX.SampleLevel(smp_bump_ds, tcd, 0);
	height += (NDetailX.w-0.5f)*0.2f;
#	endif
