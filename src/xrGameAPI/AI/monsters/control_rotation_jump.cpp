...
void CControlRotationJump::on_release()
{
    ...
    m_time_next_rotation_jump = Device.dwTimeGlobal +
        Random.randI(
            ControlRotationJumpDetails::RotationJumpDelayMin, ControlRotationJumpDetails::RotationJumpDelayMax);
}
...
bool CControlRotationJump::check_start_conditions()
{
    ...
    if (m_man->direction().is_face_target(enemy_position, ControlRotationJumpDetails::CheckYaw))
        return false;
    ...
    if (!fsimilar(m_man->movement().velocity_current(), velocity_run.velocity.linear,
            ControlRotationJumpDetails::StartSpeedDelta))
        return false;
