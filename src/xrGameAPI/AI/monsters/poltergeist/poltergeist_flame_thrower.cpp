...
CPolterFlame2::CPolterFlame2(CPoltergeist2* polter) : inherited(polter) {}

CPolterFlame::~CPolterFlame() {}
CPolterFlame2::~CPolterFlame2() {}
...
void CPolterFlame2::load(LPCSTR section)
{
    inherited::load(section);
...
void CPolterFlame2::create_flame(const CObject* target_object)
{
    Fvector position;
    if (!get_valid_flame_position(target_object, position))
        return;
...
void CPolterFlame2::select_state(SFlameElement* elem, EFlameState state)
{
    elem->state = state;
    elem->time_started = time();
...
struct remove_predicate2
{
    bool operator()(CPolterFlame::SFlameElement* element) { return (!element); }
};

struct remove_predicate3
{
    bool operator()(CPolterFlame2::SFlameElement* element) { return (!element); }
};
...
void CPolterFlame::update_schedule()
{
    ...
    m_flames.erase(std::remove_if(m_flames.begin(), m_flames.end(), remove_predicate2()), m_flames.end());
...
void CPolterFlame2::update_schedule()
{
    inherited::update_schedule();
        ...
        if (!m_state_scanning && !m_object->EnemyMan.get_enemy())
        {
            ...
            if (Actor()->Position().distance_to(m_object->Position()) < m_scan_radius)
            {
                ...
                if (m_scan_next_time < time())
                {
                    ...
                    xrAPI.Sound->play_at_pos(m_scan_sound, 0, Actor()->Position());

                    // постпроцесс
                    Actor()->Cameras().AddPPEffector(xr_new<CMonsterEffector>(m_scan_effector_info,
                        m_scan_effector_time, m_scan_effector_time_attack, m_scan_effector_time_release));
                }
            }
        }
    ...
    // удалить все элементы, выполнение которых закончено
    m_flames.erase(std::remove_if(m_flames.begin(), m_flames.end(), remove_predicate3()), m_flames.end());
