...
namespace POLTERGEIST2_REST
{
    template <typename _Object>
    class CPoltergeist2StateRest : public CStateMonsterRest<_Object>
    {
    protected:
        typedef CStateMonsterRest<_Object> inherited;

    public:
        CPoltergeist2StateRest(_Object* obj) : inherited(obj) {}
        virtual void execute();
    };

    template <typename _Object>
    void CPoltergeist2StateRest<_Object>::execute()
    {
        // check alife control
        bool captured_by_smart_terrain = false;
