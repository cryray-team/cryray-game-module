...
CPoltergeist2::CPoltergeist2()
{
    StateMan = xr_new<CStateManagerPoltergeist2>(this);
...
void CPoltergeist2::Load(LPCSTR section)
{
    inherited::Load(section);
    ...
    if (xr_strcmp(polter_type, "flamer") == 0)
    {
        m_flame = xr_new<CPolterFlame2>(this);
        m_flame->load(section);
    }
    else
    {
        m_tele = xr_new<CPolterTele2>(this);
        m_tele->load(section);
    }
}
...
void CPoltergeist2::reload(LPCSTR section)
{
    inherited::reload(section);
    Energy::reload(section, "Invisible_");
}
...
void CPoltergeist2::ForceFinalAnimation()
{
    if (state_invisible)
        anim().SetCurAnim(eAnimMiscAction_01);
}
...
void CPoltergeist2::shedule_Update(u32 dt)
{
    inherited::shedule_Update(dt);
    CTelekinesis::schedule_update();
    Energy::schedule_update();
...
BOOL CPoltergeist2::net_Spawn(CSE_Abstract* DC)
{
    if (!inherited::net_Spawn(DC))
        return (FALSE);
...
void CPoltergeist2::net_Destroy()
{
    inherited::net_Destroy();
    Energy::disable();
...
void CPoltergeist2::Die(CObject* who)
{
    if (m_tele)
    {
        if (state_invisible)
...
void CPoltergeist2::Hit(SHit* pHDS)
{
    ability()->on_hit(pHDS);
    inherited::Hit(pHDS);
}
...
void CPoltergeist2::UpdateHeight()
{
    if (!state_invisible)
        return;
...
void CPoltergeist2::on_activate()
{
    if (m_disable_hide)
        return;
...
void CPoltergeist2::on_deactivate()
{
    if (m_disable_hide)
        return;
...
{
    m_movement_manager = xr_new<POLTERGEIST_MOVEMENT_MANAGER::CPoltergeisMovementManager>(this);
...
CMovementManager* CPoltergeist2::create_movement_manager()
{
    m_movement_manager = xr_new<POLTERGEIST2_MOVEMENT_MANAGER::CPoltergeisMovementManager2>(this);
...
void CPoltergeist2::net_Relcase(CObject* O)
{
    inherited::net_Relcase(O);
    CTelekinesis::remove_links(O);
}
