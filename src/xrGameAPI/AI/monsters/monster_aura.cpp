...
void monster_aura::load_from_ini(CInifile const* ini, const char* const section, bool enable_for_dead_default)
{
    ...
    STRCONCAT(pp_effector_name_string, m_name, MonsterAuraDetails::s_pp_effector_name_string);

    char* pp_highest_at_string = NULL;
    STRCONCAT(pp_highest_at_string, m_name, MonsterAuraDetails::s_pp_highest_at_string);

    char* linear_factor_string = NULL;
    STRCONCAT(linear_factor_string, m_name, MonsterAuraDetails::s_linear_factor_string);

    char* quadratic_factor_string = NULL;
    STRCONCAT(quadratic_factor_string, m_name, MonsterAuraDetails::s_quadratic_factor_string);

    char* max_power_string = NULL;
    STRCONCAT(max_power_string, m_name, MonsterAuraDetails::s_max_power_string);

    char* max_distance_string = NULL;
    STRCONCAT(max_distance_string, m_name, MonsterAuraDetails::s_max_distance_string);

    char* sound_string = NULL;
    STRCONCAT(sound_string, m_name, MonsterAuraDetails::s_sound_string);

    char* detect_sound_string = NULL;
    STRCONCAT(detect_sound_string, m_name, MonsterAuraDetails::s_detect_sound_string);

    char* enable_for_dead_string = NULL;
    STRCONCAT(enable_for_dead_string, m_name, MonsterAuraDetails::s_enable_for_dead_string);
    ...
    const char* const sound_name = READ_IF_EXISTS(ini, r_string, section, sound_string, NULL);
    const char* const detect_sound_name = READ_IF_EXISTS(ini, r_string, section, detect_sound_string, NULL);
