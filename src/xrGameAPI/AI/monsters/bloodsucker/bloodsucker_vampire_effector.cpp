...
namespace BLOODSUCKER_VAMPIRE_EFFECTOR
{
    ...
    BOOL CVampirePPEffector::Process(SPPInfo& pp)
    {
        ...
        float factor;
        if (time_past_perc < BloodsuckerAlienVampireDetails::Time_attack)
        {
            factor = 0.75f * time_past_perc / BloodsuckerAlienVampireDetails::Time_attack;
        }
        else if (time_past_perc > (1 - BloodsuckerAlienVampireDetails::Time_attack))
        {
            factor = 0.75f * (1 - time_past_perc) / BloodsuckerAlienVampireDetails::Time_attack;
        }
        else
        {
            float time_past_sine_perc = (time_past_perc - BloodsuckerAlienVampireDetails::Time_attack) *
                (1 / (1 - BloodsuckerAlienVampireDetails::Time_attack + BloodsuckerAlienVampireDetails::Time_attack));
        ...
        : inherited(eCEVampire, time)
    {
        ...
        if (m_dist < BloodsuckerAlienVampireDetails::Best_distance)
        {
            m_direction.sub(src, tgt);
            m_dist = BloodsuckerAlienVampireDetails::Best_distance - m_dist;
        }
        else
        {
            m_direction.sub(tgt, src);
            m_dist = m_dist - BloodsuckerAlienVampireDetails::Best_distance;
        }

        m_direction.normalize();

        dangle_target.set(Random.randFs(BloodsuckerAlienVampireDetails::DeltaAngleX),
            Random.randFs(BloodsuckerAlienVampireDetails::DeltaAngleY),
            Random.randFs(BloodsuckerAlienVampireDetails::DeltaAngleZ));
    ...
    BOOL CVampireCameraEffector::ProcessCam(SCamEffectorInfo& info)
    {
        ...
        {
            if (angle_lerp(
                    dangle_current.x, dangle_target.x, BloodsuckerAlienVampireDetails::AngleSpeed, Device.fTimeDelta))
            {
                dangle_target.x = Random.randFs(BloodsuckerAlienVampireDetails::DeltaAngleX);
            }

            if (angle_lerp(
                    dangle_current.y, dangle_target.y, BloodsuckerAlienVampireDetails::AngleSpeed, Device.fTimeDelta))
            {
                dangle_target.y = Random.randFs(BloodsuckerAlienVampireDetails::DeltaAngleY);
            }

            if (angle_lerp(
                    dangle_current.z, dangle_target.z, BloodsuckerAlienVampireDetails::AngleSpeed, Device.fTimeDelta))
            {
                dangle_target.z = Random.randFs(BloodsuckerAlienVampireDetails::DeltaAngleZ);
            }
        }
