...
u32 CAI_Bloodsucker3::m_time_last_vampire = 0;
...
CAI_Bloodsucker2::CAI_Bloodsucker2()
{
    StateMan = xr_new<CStateManagerBloodsucker2>(this);
    m_alien_control.init_external(this);
    m_drag_anim_jump = false;
    m_animated = false;
    collision_off = false;
...
CAI_Bloodsucker3::CAI_Bloodsucker3()
{
    StateMan = xr_new<CStateManagerBloodsucker3>(this);
    m_alien_control.init_external(this);
...
CAI_Bloodsucker2::~CAI_Bloodsucker2() { xr_delete(StateMan); }
CAI_Bloodsucker3::~CAI_Bloodsucker3() { xr_delete(StateMan); }
...
void CAI_Bloodsucker::Load(LPCSTR section)
{
    ...
    anim().AddAnim(
        eAnimDie, "stand_idle_", -1, &velocity_none, PS_STAND, "fx_run_f", "fx_stand_b", "fx_stand_l", "fx_stand_r");
    ...
    anim().AddAnim(eAnimLookAround, "stand_look_around_", -1, &velocity_none, PS_STAND, "fx_run_f", "fx_stand_b",
        "fx_stand_l", "fx_stand_r");
    anim().AddAnim(
        eAnimSitIdle, "sit_idle_", -1, &velocity_none, PS_SIT, "fx_run_f", "fx_stand_b", "fx_stand_l", "fx_stand_r");
...
void CAI_Bloodsucker2::Load(LPCSTR section)
{
    inherited::Load(section);
    ...
    m_critical_hit_chance =
        READ_IF_EXISTS(pSettings, r_float, section, "critical_hit_chance", default_critical_hit_chance);

    m_critical_hit_camera_effector_angle = READ_IF_EXISTS(
        pSettings, r_float, section, "critical_hit_camera_effector_angle", default_critical_hit_camera_effector_angle);

    m_camera_effector_move_angular_speed = READ_IF_EXISTS(
        pSettings, r_float, section, "camera_effector_move_angular_speed", default_camera_effector_move_angular_speed);

    m_invisibility_activate_delay =
        READ_IF_EXISTS(pSettings, r_u32, section, "invisibility_activate_delay", default_invisibility_activate_delay);
}

void CAI_Bloodsucker3::Load(LPCSTR section)
{
    inherited::Load(section);
...
void CAI_Bloodsucker3::reinit()
{
    ...
    com_man().ta_fill_data(
        anim_triple_vampire, "vampire_0", "vampire_1", "vampire_2", TA_EXECUTE_LOOPED, TA_DONT_SKIP_PREPARE, 0);
...
void CAI_Bloodsucker2::reload(LPCSTR section)
{
    inherited::reload(section);
    sound().add(pSettings->r_string(section, "Sound_Invisibility_Change_State"), DEFAULT_SAMPLE_COUNT,
        SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eNormalPriority, MonsterSound::eChannelIndependent << 1,
...
void CAI_Bloodsucker3::reload(LPCSTR section)
{
    inherited::reload(section);
...
void CAI_Bloodsucker3::LoadVampirePPEffector(LPCSTR section)
{
    pp_vampire_effector.duality.h = pSettings->r_float(section, "duality_h");
    pp_vampire_effector.duality.v = pSettings->r_float(section, "duality_v");
    pp_vampire_effector.gray = pSettings->r_float(section, "gray");
...
void CAI_Bloodsucker2::BoneCallback(CBoneInstance* B)
{
    CAI_Bloodsucker2* this_class = static_cast<CAI_Bloodsucker2*>(B->callback_param());

    this_class->Bones.Update(B, Device.dwTimeGlobal);
}

void CAI_Bloodsucker3::BoneCallback(CBoneInstance* B)
{
    CAI_Bloodsucker3* this_class = static_cast<CAI_Bloodsucker3*>(B->callback_param());
...
void CAI_Bloodsucker2::vfAssignBones()
{
...
void CAI_Bloodsucker3::vfAssignBones()
{
    // ”становка callback на кости
...
{
    Actor()->Cameras().AddCamEffector(xr_new<BLOODSUCKER_VAMPIRE_EFFECTOR::CVampireCameraEffector>(
        6.0f, get_head_position(this), get_head_position(Actor())));
    Actor()->Cameras().AddPPEffector(xr_new<BLOODSUCKER_VAMPIRE_EFFECTOR::CVampirePPEffector>(pp_vampire_effector, 6.0f));
}

void CAI_Bloodsucker3::ActivateVampireEffector()
{
    Actor()->Cameras().AddCamEffector(xr_new<BLOODSUCKER3_VAMPIRE_EFFECTOR::CVampireCameraEffectorsoc>(
        6.0f, get_head_position(this), get_head_position(Actor())));
    Actor()->Cameras().AddPPEffector(
        xr_new<BLOODSUCKER3_VAMPIRE_EFFECTOR::CVampirePPEffectsocor>(pp_vampire_effector, 6.0f));
}

bool CAI_Bloodsucker::WantVampire() { return !!fsimilar(m_vampire_want_value, 1.f); }
bool CAI_Bloodsucker3::WantVampire() { return !!fsimilar(m_vampire_want_value, 1.f); }
...
void CAI_Bloodsucker3::SatisfyVampire()
{
    m_vampire_want_value = 0.f;
...
void CAI_Bloodsucker2::CheckSpecParams(u32 spec_params)
{
    if ((spec_params & ASP_CHECK_CORPSE) == ASP_CHECK_CORPSE)
    {
        com_man().seq_run(anim().get_motion_id(eAnimCheckCorpse));
...
void CAI_Bloodsucker3::CheckSpecParams(u32 spec_params)
{
    if ((spec_params & ASP_CHECK_CORPSE) == ASP_CHECK_CORPSE)
    {
        com_man().seq_run(anim().get_motion_id(eAnimCheckCorpse));
...
BOOL CAI_Bloodsucker2::net_Spawn(CSE_Abstract* DC)
{
    if (!inherited::net_Spawn(DC))
        return (FALSE);
...
void CAI_Bloodsucker2::apply_actor_camera_effector()
{
    if (!m_camera_effector_start_move_tick ||
        m_camera_effector_last_move_tick >= m_camera_effector_start_move_tick + m_camera_effector_move_time_milis)
    {
        return;
    }
    ...
    const float relative_time2 =
        (current_tick - m_camera_effector_start_move_tick) / (float)m_camera_effector_move_time_milis;
    ...
    Actor()->cam_Active()->Move(
        m_camera_effector_hor_angle > 0 ? kRIGHT : kLEFT, _abs(m_camera_effector_hor_angle) * angle_relative);

    Actor()->cam_Active()->Move(
        m_camera_effector_ver_angle > 0 ? kUP : kDOWN, _abs(m_camera_effector_ver_angle) * angle_relative);
}

BOOL CAI_Bloodsucker3::net_Spawn(CSE_Abstract* DC)
{
    if (!inherited::net_Spawn(DC))
        return (FALSE);
...
void CAI_Bloodsucker2::UpdateCL()
{
    inherited::UpdateCL();
    CControlledActor::frame_update();
    character_physics_support()->movement()->CollisionEnable(!is_collision_off());
...
void CAI_Bloodsucker3::UpdateCL()
{
    inherited::UpdateCL();
    CControlledActor::frame_update();
...
void CAI_Bloodsucker2::shedule_Update(u32 dt)
{
    inherited::shedule_Update(dt);
...
void CAI_Bloodsucker3::shedule_Update(u32 dt)
{
    inherited::shedule_Update(dt);
...
void CAI_Bloodsucker2::Die(CObject* who)
{
    inherited::Die(who);
    stop_invisible_predator();
}

void CAI_Bloodsucker3::Die(CObject* who)
{
    predator_stop();
    inherited::Die(who);
}

void CAI_Bloodsucker::post_fsm_update() { inherited::post_fsm_update(); }
void CAI_Bloodsucker2::post_fsm_update() { inherited::post_fsm_update(); }
void CAI_Bloodsucker3::post_fsm_update() { inherited::post_fsm_update(); }
...
bool CAI_Bloodsucker2::check_start_conditions(ControlCom::EControlType type)
{
    if (type == ControlCom::eControlJump)
    {
        return false;
...
bool CAI_Bloodsucker3::check_start_conditions(ControlCom::EControlType type)
{
    if (!inherited::check_start_conditions(type))
        return false;
...
void CAI_Bloodsucker2::set_alien_control(bool val) { val ? m_alien_control.activate() : m_alien_control.deactivate(); }
void CAI_Bloodsucker3::set_alien_control(bool val) { val ? m_alien_control.activate() : m_alien_control.deactivate(); }
...
void CAI_Bloodsucker2::set_invis()
{
    m_vis_state = -1;
    predator_start();
}
...
void CAI_Bloodsucker2::set_collision_off(bool b_collision) { collision_off = b_collision; }

bool CAI_Bloodsucker::is_collision_off() { return collision_off; }
bool CAI_Bloodsucker2::is_collision_off() { return collision_off; }
...
void CAI_Bloodsucker2::jump(const Fvector& position, float factor)
{
    com_man().script_jump(position, factor);
    sound().play(MonsterSound::eMonsterSoundAggressive);
}
...
void CAI_Bloodsucker2::set_drag_jump(CEntityAlive* e, LPCSTR s, const Fvector& position, float factor)
{
    j_position = position;
    j_factor = factor;
    m_cob = e;
...
bool CAI_Bloodsucker2::is_drag_anim_jump() { return m_drag_anim_jump; }

bool CAI_Bloodsucker::is_animated() { return m_animated; }
bool CAI_Bloodsucker2::is_animated() { return m_animated; }
...
void CAI_Bloodsucker2::start_drag()
{
    if (m_animated)
    {
        com_man().script_capture(ControlCom::eControlAnimation);
...
void CAI_Bloodsucker2::animation_end_jump(CBlend* B)
{
    ((CAI_Bloodsucker2*)B->CallbackParam)->set_invis();
    ((CAI_Bloodsucker2*)B->CallbackParam)
        ->jump(((CAI_Bloodsucker2*)B->CallbackParam)->j_position, ((CAI_Bloodsucker2*)B->CallbackParam)->j_factor);
}
...
void CAI_Bloodsucker2::predator_start()
{
    if (m_vis_state != 0)
    {
        if (m_vis_state == 1)
    ...
    CParticlesPlayer::StartParticles(invisible_particle_name, Fvector().set(0.0f, 0.1f, 0.0f), ID());
    sound().play(CAI_Bloodsucker2::eChangeVisibility);

    m_predator = true;
}

void CAI_Bloodsucker3::predator_start()
{
    if (m_predator)
        return;
    ...
    CParticlesPlayer::StartParticles(invisible_particle_name, Fvector().set(0.0f, 0.1f, 0.0f), ID());
    sound().play(CAI_Bloodsucker3::eChangeVisibility);
...
void CAI_Bloodsucker2::predator_stop()
{
    if (m_vis_state != 0)
    {
        if (m_vis_state == -1)
...
void CAI_Bloodsucker3::predator_stop()
{
    if (!m_predator)
        return;
...
void CAI_Bloodsucker2::predator_freeze() { control().animation().freeze(); }
void CAI_Bloodsucker3::predator_freeze() { control().animation().freeze(); }

void CAI_Bloodsucker::predator_unfreeze() { control().animation().unfreeze(); }
void CAI_Bloodsucker2::predator_unfreeze() { control().animation().unfreeze(); }
void CAI_Bloodsucker3::predator_unfreeze() { control().animation().unfreeze(); }
...
void CAI_Bloodsucker2::move_actor_cam(float angle)
{
    if (Actor()->cam_Active())
    {
        Actor()->cam_Active()->Move(Random.randI(2) ? kRIGHT : kLEFT, angle);
...
void CAI_Bloodsucker3::move_actor_cam()
{
...
void CAI_Bloodsucker2::HitEntity(const CEntity* pEntity, float fDamage, float impulse, Fvector& dir)
{
    bool is_critical = rand() / (float)RAND_MAX <= m_critical_hit_chance;
...
void CAI_Bloodsucker3::HitEntity(const CEntity* pEntity, float fDamage, float impulse, Fvector& dir)
{
    inherited::HitEntity(pEntity, fDamage, impulse, dir);
...
bool CAI_Bloodsucker2::in_solid_state() { return !state_invisible; }
...
void CAI_Bloodsucker2::Hit(SHit* pHDS)
{
    if (state_invisible)
    {
        return;
...
void CAI_Bloodsucker2::start_invisible_predator()
{
    state_invisible = true;
    predator_start();
}

void CAI_Bloodsucker3::start_invisible_predator()
{
    state_invisible = true;
    predator_start();
}
...
void CAI_Bloodsucker2::stop_invisible_predator()
{
    state_invisible = false;
    predator_stop();
}

void CAI_Bloodsucker3::stop_invisible_predator()
{
    state_invisible = false;
    predator_stop();
}
...
void CAI_Bloodsucker2::manual_activate()
{
    state_invisible = true;
    setVisible(FALSE);
}

void CAI_Bloodsucker3::manual_activate()
{
    state_invisible = true;
    setVisible(FALSE);
}
...
void CAI_Bloodsucker2::manual_deactivate()
{
    state_invisible = false;
    setVisible(TRUE);
}

void CAI_Bloodsucker3::manual_deactivate()
{
    state_invisible = false;
    setVisible(TRUE);
}

void CAI_Bloodsucker3::on_activate_control(ControlCom::EControlType type)
{
    if (type == ControlCom::eControlThreaten)
    {
...
bool CAI_Bloodsucker3::done_enough_hits_before_vampire()
{
    return (int)m_hits_before_vampire >=
        (int)m_sufficient_hits_before_vampire + m_sufficient_hits_before_vampire_random;
}
...
void CAI_Bloodsucker3::on_attack_on_run_hit() { ++m_hits_before_vampire; }
