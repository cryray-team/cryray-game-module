...
CAI_Dog2::CAI_Dog2()
{
    StateMan = xr_new<CStateManagerDog2>(this);
...
void CAI_Dog2::Load(LPCSTR section)
{
    inherited::Load(section);
...
void CAI_Dog2::CheckSpecParams(u32 spec_params)
{
    if ((spec_params & ASP_CHECK_CORPSE) == ASP_CHECK_CORPSE)
    {
        com_man().seq_run(anim().get_motion_id(eAnimCheckCorpse));
