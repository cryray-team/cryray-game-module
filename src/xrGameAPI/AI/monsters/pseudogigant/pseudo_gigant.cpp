...
CPseudoGigantJumper::CPseudoGigantJumper()
{
    ...
    StateMan = xr_new<CStateManagerGigantJumper>(this);
...
CPseudoGigantJumper::~CPseudoGigantJumper() { xr_delete(StateMan); }
...
void CPseudoGigant::Load(LPCSTR section)
{
    ...
    xrAPI.Sound->create(m_sound_start_threaten, pSettings->r_string(section, "sound_threaten_start"), st_Effect,
...
#define MAX_STEP_RADIUS1 60.f
#define MAX_STEP_RADIUS2 60.f
...
void CPseudoGigant::event_on_step()
{
    ...
    if (pActor)
    {
        ...
        float max_dist = MAX_STEP_RADIUS1;
        if (dist_to_actor < max_dist)
            Actor()->Cameras().AddCamEffector(xr_new<PSEUDOGIGANT_STEP_EFFECTOR::CPseudogigantStepEffector>(step_effector.time,
...
void CPseudoGigantJumper::event_on_step()
{
    ...
    {
        float dist_to_actor = pActor->Position().distance_to(Position());
        float max_dist = MAX_STEP_RADIUS2;
        if (dist_to_actor < max_dist)
            Actor()->Cameras().AddCamEffector(xr_new<PSEUDOGIGANT_JUMPER_STEP_EFFECTOR::CPseudoGigantJumperStepEffector>(step_effector.time,
                step_effector.amplitude, step_effector.period_number, (max_dist - dist_to_actor) / (1.2f * max_dist)));
    }
}
...
#ifndef ANOMALY
    if (pA)
        PseudoGigantThreatenExecuteScript();
#endif
}
