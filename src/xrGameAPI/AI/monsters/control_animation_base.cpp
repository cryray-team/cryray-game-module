...
bool CControlAnimationBase::get_animation_info(EMotionAnim anim, u32 index, MotionID& motion, float& length) const
{
    ...
    IKinematicsAnimated* animated = dynamic_cast<IKinematicsAnimated*>(m_object->Visual());
    if (!animated)
    {
        return false;
    }
...
void CControlAnimationBase::select_animation(bool anim_end)
{
    ...
    // установить анимацию
    string128 s1, s2;
    MotionID cur_anim = dynamic_cast<IKinematicsAnimated*>(m_object->Visual())
...
void CControlAnimationBase::FX_Play(EHitSide side, float amount)
{
    ...
    if (p_str && p_str->size())
        dynamic_cast<IKinematicsAnimated*>(m_object->Visual())->PlayFX(*(*p_str), amount);
...
MotionID CControlAnimationBase::get_motion_id(EMotionAnim a, u32 index)
{
    ...
    string128 s1, s2;
    return (dynamic_cast<IKinematicsAnimated*>(m_object->Visual())
                ->ID_Cycle_Safe(strconcat(sizeof(s2), s2, *anim_it->target_name, itoa(index, s1, 10))));
}
