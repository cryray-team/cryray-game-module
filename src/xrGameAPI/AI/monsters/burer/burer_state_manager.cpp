...
CStateManagerBurer::CStateManagerBurer(CBurer* monster) : inherited(monster)
{
    ...
    add_state(eStateAttack, xr_new<BURER_STATE_ATTACK::CStateBurerAttack<CBurer>>(monster));
...
CStateManagerBurer2::CStateManagerBurer2(CBurer2* monster) : inherited(monster)
{
    add_state(eStateRest, xr_new<CStateMonsterRest<CBurer2>>(monster));
    add_state(eStatePanic, xr_new<CStateMonsterPanic<CBurer2>>(monster));
    add_state(eStateAttack, xr_new<BURER2_STATE_ATTACK::CStateBurer2Attack<CBurer2>>(monster));
    add_state(eStateEat, xr_new<CStateMonsterEat<CBurer2>>(monster));
    add_state(eStateHearInterestingSound, xr_new<CStateMonsterHearInterestingSound<CBurer2>>(monster));
    add_state(eStateHearDangerousSound, xr_new<CStateMonsterHearDangerousSound<CBurer2>>(monster));
    add_state(eStateHitted, xr_new<CStateMonsterHitted<CBurer2>>(monster));
    add_state(eStateBurerScanning, xr_new<CStateMonsterCustomAction<CBurer2>>(monster));
}

CStateManagerBurer::~CStateManagerBurer() {}
CStateManagerBurer2::~CStateManagerBurer2() {}

#define SCAN_STATE_TIME1 4000
#define SCAN_STATE_TIME2 4000
...
void CStateManagerBurer::execute()
{
    ...
    else if (object->time_last_scan + SCAN_STATE_TIME1 > Device.dwTimeGlobal)
...
void CStateManagerBurer2::execute()
{
    u32 state = u32(-1);
    ...
    else if (object->time_last_scan + SCAN_STATE_TIME2 > Device.dwTimeGlobal)
    {
        state = eStateBurerScanning;
    }
    else if (can_eat())
...
void CStateManagerBurer2::setup_substates()
{
    if (current_substate == eStateBurerScanning)
    {
        SStateDataAction data;
