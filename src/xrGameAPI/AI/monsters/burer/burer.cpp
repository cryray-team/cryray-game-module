...
bool CBurer2::can_scan = true;

CBurer::CBurer()
{
    StateMan = xr_new<CStateManagerBurer>(this);

    m_fast_gravi = xr_new<BURER_FAST_GRAVI::CBurerFastGravi>();
...
CBurer2::CBurer2()
{
    StateMan = xr_new<CStateManagerBurer2>(this);
    TScanner::init_external(this);

    m_fast_gravi = xr_new<BURER2_FAST_GRAVI::CBurer2FastGravi>();
    control().add(m_fast_gravi, ControlCom::eComCustom1);
}
...
void CBurer2::reload(LPCSTR section)
{
    inherited::reload(section);

    // add specific sounds
    sound().add(pSettings->r_string(section, "sound_gravi_attack"), DEFAULT_SAMPLE_COUNT, SOUND_TYPE_MONSTER_ATTACKING,
        MonsterSound::eHighPriority + 2, u32(MonsterSound::eBaseChannel), eMonsterSoundGraviAttack, "bip01_head");
    sound().add(pSettings->r_string(section, "sound_tele_attack"), DEFAULT_SAMPLE_COUNT, SOUND_TYPE_MONSTER_ATTACKING,
        MonsterSound::eHighPriority + 3, u32(MonsterSound::eBaseChannel), eMonsterSoundTeleAttack, "bip01_head");
...
void CBurer::Load(LPCSTR section)
{
    ...
    xrAPI.Sound->create(
        sound_gravi_wave, pSettings->r_string(section, "sound_gravi_wave"), st_Effect, SOUND_TYPE_WORLD);
    xrAPI.Sound->create(sound_tele_hold, pSettings->r_string(section, "sound_tele_hold"), st_Effect, SOUND_TYPE_WORLD);
    xrAPI.Sound->create(
        sound_tele_throw, pSettings->r_string(section, "sound_tele_throw"), st_Effect, SOUND_TYPE_WORLD);
...
void CBurer2::Load(LPCSTR section)
{
    inherited::Load(section);
    ...
    anim().AddAnim(
        eAnimDie, "stand_die_", -1, &velocity_none, PS_STAND, "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r");
    ...
    anim().AddAnim(eAnimSteal, "stand_steal_", -1, &velocity_steal, PS_STAND, "fx_stand_f", "fx_stand_b", "fx_stand_l",
        "fx_stand_r");
    anim().AddAnim(
        eAnimEat, "sit_eat_", -1, &velocity_none, PS_SIT, "fx_stand_f", "fx_stand_b", "fx_stand_l", "fx_stand_r");
...
void CBurer2::shedule_Update(u32 dt)
{
    inherited::shedule_Update(dt);
...
void CBurer2::CheckSpecParams(u32 spec_params) {}
...
void CBurer::UpdateGraviObject()
{
    ...
    if (sound_gravi_wave._feedback())
    {
        ...
        xrAPI.Sound->play_at_pos(sound_gravi_wave, 0, snd_pos);
}

void CBurer2::UpdateGraviObject()
{
    if (!m_gravi_object.active)
        return;
...
void CBurer2::StartGraviPrepare()
{
    const CEntityAlive* enemy = EnemyMan.get_enemy();
    if (!enemy)
        return;
...
void CBurer2::on_scanning() { time_last_scan = Device.dwTimeGlobal; }
...
void CBurer2::net_Relcase(CObject* O)
{
    inherited::net_Relcase(O);
