...
#include "grenade.h"
#else
#include "../xrGameAPI_IWP/WeaponHudBase/Grenade.h"
#endif

#include "Include/UIRender.h"
#include "Include/UIShader.h"

u32 CHitMarker::color_local = 0;
//--------------------------------------------------------------------
...
void CHitMarker::Render()
{
    ...
    pCRRenderData->hit_power_factor = color_get_A(color_local);
...
{
    int frame;
    CHitMarker::color_local = m_lanim->CalculateRGB(Device.fTimeGlobal - m_StartTime, frame);
    m_UIStaticItem->SetTextureColor(
        subst_alpha(m_UIStaticItem->GetTextureColor(), color_get_A(CHitMarker::color_local)));
