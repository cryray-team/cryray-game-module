...
void CInventoryItem::Load(LPCSTR section)
{
    ...
    else
    {
        ...
        if (FS.m_Flags.test(FS.flPrintLTX))
            Msg("'kind' for section %s doesn't exist!", section);
    }
...
void CInventoryItem::OnMoveToRuck(const SInvItemPlace& prev)
{
    if (m_pInventory && Actor()->m_inventory == m_pInventory)
    {
        Actor()->CreateNightVision();
        if (Actor()->GetNightVision()->m_section == m_section_id)
            Actor()->SetNightVisionStatus(false, true);
    }
}