...
extern ENGINE_API float debug_ps_sun_shafts_value_v;
...
//u32 render_get_dx_level() { return xrAPI.Render->get_dx_level(); }

GAME_API CUISequencer* g_tutorial = NULL;
GAME_API CUISequencer* g_tutorial2 = NULL;
...
void set_weather_value_numric(LPCSTR name, float val)
{
    ...
    else if (0 == xr_strcmp(name, "sun_shafts_intensity"))
    {
        ...
        E.m_fSunShaftsIntensity *= 1.0f - debug_ps_sun_shafts_min_v;
        E.m_fSunShaftsIntensity += debug_ps_sun_shafts_min_v;
        E.m_fSunShaftsIntensity *= debug_ps_sun_shafts_value_v;
        ...
        Msg("~ [%s] is not a valid numric weather parameter to set", name);
}
...
void set_weather_value_vector(LPCSTR name, float x, float y, float z, float w = 0)
{
    ...
    if (0 == xr_strcmp(name, "sky_color"))
        ...
        Msg("[%s] is not a valid vector weather parameter to set", name);
    // std::format()
}
...
void set_weather_value_string(LPCSTR name, LPCSTR newval)
{
    ...
    else if (0 == xr_strcmp(name, "thunderbolt_collection"))
    {
        E.tb_id = environment()->eff_Thunderbolt->AppendDef(*environment(),
            environment()->m_thunderbolt_collections_config, environment()->m_thunderbolts_config, newval);
    }
    else if (0 == xr_strcmp(name, "ambient"))
    {
        ...
        Msg("~ [%s] is not a valid string weather parameter to set", name);
}

void pause_weather(bool b_pause) { environment()->m_paused = b_pause; }

bool is_weather_paused() { return environment()->m_paused; }

void reload_weather() { environment()->Reload(); }
...
void boost_weather_value(LPCSTR name, float value)
{
    if (0 == xr_strcmp(name, "ambient_color"))
        ...
        Msg("~ [%s] is not a valid weather parameter to boost", name);
}
...
void hud_adj_state(bool state) { g_player_hud->m_adjust_mode = state; }

LPCSTR vid_modes_string()
{
    xr_string resolutions = "";

    xr_token* tok = xrAPI.vid_mode_token;
    while (tok->name)
    {
...
void set_actor_allow_ladder(bool b) { CryRayAPI.g_actor_allow_ladder = b; }

void set_nv_lumfactor(float factor) { g_pGamePersistent->nv_shader_data.lum_factor = factor; }
...
const u32 ActorMovingState() { return g_actor->MovingState(); }
...
void reload_hud_xml() { HUD().OnScreenResolutionChanged(); }
...
void prefetch_texture(LPCSTR name) { Device.m_pRender->ResourcesPrefetchCreateTexture(name); }

void prefetch_model(LPCSTR name) { xrAPI.Render->models_PrefetchOne(name); }
//-Alundaio
...
namespace /*unity*/
{
    struct script_rq_result
    {
        ...
        void set_rq(collide::rq_result& R)
        {
            if (R.O)
            {
...
void CLevel::script_register(lua_State* L)
{
    ...
    // Alundaio: Extend level namespace exports
        def("patrol_path_add", &patrol_path_add), def("patrol_path_remove", &patrol_path_remove),
        ...
        def("CryRayTimer", CRTimers::TimerChainID), 
        ...
        def("get_rain_volume", &get_rain_volume), def("set_snd_volume", &set_snd_volume),
        ...
        def("press_action", &LevelPressAction), def("release_action", &LevelReleaseAction),
        def("hold_action", &LevelHoldAction),

        def("actor_moving_state", &ActorMovingState), def("get_env_rads", &get_env_rads),
    ...
    module(L, "weather")[def("get_value_numric", get_weather_value_numric),
        def("get_value_vector", get_weather_value_vector), def("get_value_string", get_weather_value_string),
        def("pause", pause_weather), def("is_paused", is_weather_paused),
        def("set_value_numric", set_weather_value_numric), def("set_value_vector", set_weather_value_vector),
        def("set_value_string", set_weather_value_string), def("reload", reload_weather),
        def("boost_value", boost_weather_value), def("boost_reset", boost_weather_reset), def("sun_time", sun_time)];

    module(L, "hud_adjust")[def("enabled", hud_adj_state), def("set_vector", hud_adj_offs),
        def("set_value", hud_adj_value), def("remove_hud_model", remove_hud_model)];
    ...
    module(L, "game")[class_<xrTime>("CTime")
                          ...
                          .def("set", &xrTime::set)
                          .def("get", &xrTime::get,
                              out_value(_2xxx) + out_value(_3xxx) + out_value(_4xxx) + out_value(_5xxx) + out_value(_6xxx) +
                                  out_value(_7xxx) + out_value(_8xxx))
        ...
        def("reload_language", &reload_language), def("get_resolutions", &vid_modes_string),
        def("play_hud_motion", PlayHudMotion), def("stop_hud_motion", StopHudMotion),
        def("get_motion_length", MotionLength), def("hud_motion_allowed", AllowHudMotion),
        def("play_hud_anm", PlayBlendAnm), def("stop_hud_anm", StopBlendAnm),
        def("stop_all_hud_anms", StopAllBlendAnms), def("set_hud_anm_time", SetBlendAnmTime),
        def("only_allow_movekeys", block_all_except_movement), def("only_movekeys_allowed", only_movement_allowed),
        def("set_actor_allow_ladder", set_actor_allow_ladder), def("set_nv_lumfactor", set_nv_lumfactor),
        def("reload_ui_xml", reload_hud_xml), def("actor_weapon_lowered", actor_safemode),
        def("actor_lower_weapon", actor_set_safemode), def("prefetch_texture", prefetch_texture),
        def("prefetch_model", prefetch_model), def("world2ui", world2ui)];

    module(L, "Random")[def("I", &RandomInteger0), def("I", &RandomInteger1), def("I", &RandomInteger2),
        def("F", &RandomFloat0), def("F", &RandomFloat1), def("F", &RandomFloat2)];
}
