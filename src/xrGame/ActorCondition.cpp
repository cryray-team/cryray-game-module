...
void CActorCondition::LoadCondition(LPCSTR entity_section)
{
    ...
    m_fSleepCritical =
        READ_IF_EXISTS(pSettings, r_float, section, "sleep_critical", (pSGameSettings->bEnableSleep) ? 0.35f : 0.0f);
    clamp(m_fSleepCritical, 0.0f, 1.0f);
    m_fV_Sleep =
        READ_IF_EXISTS(pSettings, r_float, section, "sleep_v", (pSGameSettings->bEnableSleep) ? 0.0000350f : 0.0f);
    m_fV_SleepPower =
        READ_IF_EXISTS(pSettings, r_float, section, "sleep_power_v", (pSGameSettings->bEnableSleep) ? 0.0f : 0.0f);
    m_fV_SleepHealth =
        READ_IF_EXISTS(pSettings, r_float, section, "sleep_health_v", (pSGameSettings->bEnableSleep) ? 0.0f : 0.0f);

    m_fThirstCritical =
        READ_IF_EXISTS(pSettings, r_float, section, "thirst_critical", (pSGameSettings->bEnableThirst) ? 0.3f : 0.0f);
    clamp(m_fThirstCritical, 0.0f, 1.0f);
    m_fV_Thirst =
        READ_IF_EXISTS(pSettings, r_float, section, "thirst_v", (pSGameSettings->bEnableThirst) ? 0.0000255f : 0.0f);
    m_fV_ThirstPower = READ_IF_EXISTS(
        pSettings, r_float, section, "thirst_power_v", (pSGameSettings->bEnableThirst) ? 0.00025f : 0.0f);
    m_fV_ThirstHealth =
        READ_IF_EXISTS(pSettings, r_float, section, "thirst_health_v", (pSGameSettings->bEnableThirst) ? 0.0f : 0.0f);

    m_MaxWalkWeight = token_weight + 10.f; //-' Основной вес + 10 с перегрузом
    m_CarryWeightBoost = 0.0f;
        ...
        if ((m_fAlcohol > 0.0001f))
        {
            if (!ce)
            {
                AddEffector(m_object, effAlcohol, "effector_alcohol",
                    fastdelegate::MakeDelegate(this, &CActorCondition::GetAlcohol));
            }
        }
        else
        {
            ...
            if (!fsimilar(GetPsyHealth(), 1.0f, 0.05f))
            {
                if (!ppe)
                {
                    AddEffector(m_object, effPsyHealth, pp_sect_name,
                        fastdelegate::MakeDelegate(this, &CActorCondition::GetPsy));
                }
            }
            else
            {
...
void CActorCondition::UpdateArtefactReaction() { inherited::UpdateArtefactReaction(); }
...
void CActorCondition::UpdateSleep()
{
    ...
    float sleep_health_koef =
        (m_fSleep - m_fSleepCritical) / (m_fSleep >= m_fSleepCritical ? 1 - m_fSleepCritical : m_fSleepCritical);
    if (CanBeHarmed() && !psActorFlags.test(AF_GODMODE_RT))
    {
...
bool CActorCondition::IsSleeping() const { return object().HasInfo("actor_is_sleeping"); }
...
void CActorCondition::ChangeSatiety(float value) { m_fSatietyChange += value; }
