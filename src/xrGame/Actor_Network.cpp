...
GAME_API CActor* g_actor = nullptr;
...
void CActor::net_ExportDeadBody(NET_Packet& P)
{
    ...
    for (u16 i = 0; i < bones_number; i++)
    {
        ...
        xray_q8::w_vec_q8(P, state.position, min, max);
        xray_q8::w_qt_q8(P, state.quaternion);

        //---------------------------------
        Fvector px = state.linear_vel;
        px.div(10.0f);
        px.add(state.position);
        xray_q8::w_vec_q8(P, px, min, max);
    };
};
...
void CActor::net_Import_Physic(NET_Packet& P)
{
    ...
    if (m_u16NumBones != 1)
    {
        ...
        for (u16 i = 0; i < m_u16NumBones; i++)
        {
            ...
            xray_q8::r_vec_q8(P, stateL.position, min, max);
            xray_q8::r_qt_q8(P, stateL.quaternion);
            //---------------------------------------
            xray_q8::r_vec_q8(P, stateL.linear_vel, min, max);
...
void CActor::save(NET_Packet& output_packet)
{
    ...
    output_packet.w_u8(m_bNightVisionOn);
}
...
void CActor::load(IReader& input_packet)
{
    ...
    m_bNightVisionOn = !!input_packet.r_u8();
