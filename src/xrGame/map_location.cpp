...
void CMapLocation::UpdateSpot(UICustomMap::CUICustomMap* map, CMapSpot* sp)
{
...
void CMapLocation::UpdateSpotPointer(UICustomMap::CUICustomMap* map, CMapSpotPointer* sp)
{
    if (sp->GetParent())
...
void CMapLocation::UpdateMiniMap(UICustomMap::CUICustomMap* map)
{
...
void CMapLocation::UpdateLevelMap(UICustomMap::CUICustomMap* map)
{
...
bool CRelationMapLocation::Update()
{
    ...
    if (m_last_relation == ALife::eRelationTypeEnemy || m_last_relation == ALife::eRelationTypeWorstEnemy)
    {
        ...
        if (_object_)
        {
            ...
            {
                const CGameObject* pObj = dynamic_cast<const CGameObject*>(_object_);
                CActor* pAct = dynamic_cast<CActor*>(Level().Objects.net_Find(m_pInvOwnerActorID));
                CHelmet* helm = dynamic_cast<CHelmet*>(pAct->inventory().ItemFromSlot(HELMET_SLOT));
...
void CRelationMapLocation::UpdateMiniMap(UICustomMap::CUICustomMap* map)
{
    if (IsVisible() && m_b_minimap_visible)
        inherited::UpdateMiniMap(map);
}

void CRelationMapLocation::UpdateLevelMap(UICustomMap::CUICustomMap* map)
{
