...
void CLevel::g_sv_Spawn(CSE_Abstract* E)
{
    ...
    if (pStructCryRay->dbgnetspawn)
        Msg("~ Try Spawning object Name:[%s] Section:[%s] ID:[%d] ParentID:[%d]", E->name_replace(),
            *E->s_name, E->ID, E->ID_Parent);

    //	T.Start		();
    if (O == 0)
    {
        Msg("! M1^ Failed to spawn entity '%s'", *E->s_name);
    }
    else if (!O->net_Spawn(E)) 
    {
        ...
        Objects.Destroy(O);
        Msg("! M2^ Failed to spawn entity '%s'", *E->s_name);
    }
    else
    {
    ...
    if (strstr(Core.Params, "-level_network_spawn"))
