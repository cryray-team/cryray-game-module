...
float CVisualMemoryManager::get_visible_value(const CGameObject* game_object, float distance, float object_distance,
    float time_delta, float object_velocity, float luminocity, float trans) const
{
    ...
    float fog = (object_distance * fog_w + fog_x) * current_state2().m_fog_factor;
    clamp(fog, 0.f, 1.f);
    float fog_factor = 1.f - pow(fog, current_state2().m_fog_pow);
    ...
    if (ai().script_engine().functor("visual_memory_manager.get_visible_value", funct))
        return (funct(m_object ? m_object->lua_game_object() : 0, game_object ? game_object->lua_game_object() : 0,
            time_delta, current_state2().m_time_quant, luminocity, current_state2().m_velocity_factor, object_velocity,
            distance, object_distance, always_visible_distance));
    //-Alundaio

    return (time_delta / current_state2().m_time_quant * luminocity *
        (1.f + current_state2().m_velocity_factor * object_velocity) * (distance - object_distance) /
