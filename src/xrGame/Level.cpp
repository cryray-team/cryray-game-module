...
shared_str CLevel::name_translated() const { return CStringTable().translate(map_data.m_name); }
...

    {
        if (g_mt_config.test(mtLevelSounds))
        {
            Device.seqParallel.push_back(
                fastdelegate::MakeDelegate(m_level_sound_manager, &CLevelSoundManager::Update));
        }
        else
...
{
    xrAPI.Render->BeforeWorldRender();
    ...
    if (game && CurrentGameUI() && &CurrentGameUI()->GetPdaMenu() != nullptr)
    {
        ...
        if (psActorFlags.test(AF_3D_PDA) && pda->IsShown())
        {
            ...
            if (cursor)
            {
                ...
                if (pda->IsEnabled() && is_top && !Console->isOpen())
                {
                    if (need_reset)
                    {
                        need_reset = false;
            ...
            xrAPI.Render->RenderToTarget(xrAPI.Render->rtPDA);
        }
...
float CLevel::GetGameTimeFactor() { return (game ? game->GetGameTimeFactor() : 1.0f); }
