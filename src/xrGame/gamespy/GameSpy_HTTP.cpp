...
void CGameSpy_HTTP::StartUp() { xrGS_ghttpStartup(); }

void CGameSpy_HTTP::CleanUp() { xrGS_ghttpCleanup(); }

void CGameSpy_HTTP::Think() { xrGS_ghttpThink(); }
    ...
    GHTTPRequest request, GHTTPResult result, char* buffer, GHTTPByteCount bufferLen, void* param)
{
    switch (result)
    {
    case GHTTPSuccess: MainMenu()->OnDownloadPatchSuccess(); break;
    default:
