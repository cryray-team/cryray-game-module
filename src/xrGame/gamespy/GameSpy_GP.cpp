...
{
    return xrGS_gpNewUserA(&m_GPConnection, nick.c_str(), unique_nick.c_str(), email.c_str(), password.c_str(), NULL,
...
{
    return xrGS_gpProfileSearchA(&m_GPConnection, nick.c_str(), unique_nick.c_str(), email.c_str(), NULL, NULL, 0,
        GP_NON_BLOCKING, callback, param);
}
...
void CGameSpy_GP::Disconnect() { xrGS_gpDisconnect(&m_GPConnection); }
