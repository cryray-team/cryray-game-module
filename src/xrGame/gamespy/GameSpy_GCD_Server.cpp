...
void CGameSpy_GCD_Server::ShutDown() { xrGS_gcd_shutdown(); }
...
void CGameSpy_GCD_Server::DisconnectUser(int localid) { xrGS_gcd_disconnect_user(localid); };

void CGameSpy_GCD_Server::Think() { xrGS_gcd_think(); };

char* CGameSpy_GCD_Server::GetKeyHash(int localid) { return xrGS_gcd_getkeyhash(localid); };
