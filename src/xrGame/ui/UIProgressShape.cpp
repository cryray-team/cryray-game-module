...
UIProgressShape::CUIProgressShape::CUIProgressShape()
{
    m_bText = false;
    m_blend = true;
    m_angle_begin = 0.0f;
...
void UIProgressShape::CUIProgressShape::SetPos(float pos) { m_stage = pos; }

void UIProgressShape::CUIProgressShape::SetPos(int pos, int max)
{
...
void UIProgressShape::CUIProgressShape::SetTextVisible(bool b) { m_bText = b; }
...
void UIProgressShape::CUIProgressShape::Draw()
{
    if (m_bText)
        DrawText();

    xrAPI.UIRender->SetShader(*GetShader());
    Fvector2 tsize;
    xrAPI.UIRender->GetActiveTextureResolution(tsize);

    xrAPI.UIRender->StartPrimitive(m_sectorCount * 3, IUIRender::ptTriList, UI().m_currentPointType);
    ...
    for (u32 i = 0; i < m_sectorCount; ++i)
    {
        ...
        xrAPI.UIRender->PushPoint(center_pos.x, center_pos.y, 0, color, center_tex.x, center_tex.y);

        Fvector2 UIProgressShape_tp;
        UIProgressShape_tp.set(prev_pos_pt);
        UIProgressShape_tp.add(center_pos);

        Fvector2 UIProgressShape_tx;
        UIProgressShape_tx.set(prev_tex_pt);
        UIProgressShape_tx.add(center_tex);

        Fvector2 tp1;
        Fvector2 tx1;
        tp1.set(UIProgressShape_tp);
        tx1.set(UIProgressShape_tx);
        ...
        if (m_bClockwise)
        {
            xrAPI.UIRender->PushPoint(tp1.x, tp1.y, 0, color, tx1.x, tx1.y);
            xrAPI.UIRender->PushPoint(UIProgressShape_tp.x, UIProgressShape_tp.y, 0, color, UIProgressShape_tx.x, UIProgressShape_tx.y);
        }
        else
        {
            xrAPI.UIRender->PushPoint(UIProgressShape_tp.x, UIProgressShape_tp.y, 0, color, UIProgressShape_tx.x, UIProgressShape_tx.y);
