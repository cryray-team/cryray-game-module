...
void CUIOptionsManager::SendMessage2Group(const shared_str& group, const char* message)
{
    ...
    CRASH_PROTECTION(m_groups.end() != it, "! [CryRay Engine x64 INFO]: Invalid settings group [%s] found in options!",
        group.c_str());
...
void CUIOptionsManager::DoLevelRestart() { m_restart_flags |= e_level_restart; }