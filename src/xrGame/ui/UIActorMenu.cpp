...
void CUIActorMenu::SetMenuMode(EMenuMode mode)
{
    ...
    if (mode != m_currMenuMode)
    {
        ...
        case mmDeadBodySearch: 
            if ((m_pPartnerInvOwner && m_pPartnerInvOwner->inventory().ModifyFrame() != m_trade_partner_inventory_state) && update_loot_pick)
                RefreshDeadBodyInventoryContents();

            DeInitDeadBodySearchMode(); break;
...
void CUIActorMenu::Show(bool status)
{
    ...
    if (status)
    {
        if (p_engine_flags32.test(AF_HAND_HIDE_INVENTORY))
...
void CUIActorMenu::InfoCurItem(CUICellItem* cell_item)
{
    ...
    {
        CInventoryOwner* item_owner = dynamic_cast<CInventoryOwner*>(current_item->m_pInventory->GetOwner());
        ...
        /* CWeaponAmmo* ammo = dynamic_cast<CWeaponAmmo*>(current_item);
        if(ammo)
        {
            for( u32 j = 0; j < cell_item->ChildsCount(); ++j )
            {
                ...
                CInventoryOwner* ammo_owner = dynamic_cast<CInventoryOwner*>(jitem->m_pInventory->GetOwner());
...
void CUIActorMenu::highlight_item_slot(CUICellItem* cell_item)
{
    ...
    CEatableItem* eatable = dynamic_cast<CEatableItem*>(item);
    if (eatable)
    {
...
void CUIActorMenu::set_highlight_item(CUICellItem* cell_item)
{
    ...
    // Check if object is active, name and section is valid. Inventory can be nullptr, because this can be container
    // (not dead body, or trader)
