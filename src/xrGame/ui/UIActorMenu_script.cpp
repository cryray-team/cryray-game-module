...
void CUIActorMenu::TryRepairItem(CUIWindow* w, void* d)
{
    ...
    luabind::functor<bool> funct;
    R_ASSERT2(ai().script_engine().functor("inventory_upgrades.can_afford_repair_item", funct),
...
void CUIActorMenu::script_register(lua_State* L)
{
    module(
        ...
        class_<CUIHudStatesWnd, CUIWindow>("CUIHudStatesWnd")
            ...
            .def_readonly("m_ui_psy_bar", &CUIHudStatesWnd::m_ui_psy_health_bar)
            ...
            .def_readwrite("m_ui_psy_bar_show", &CUIHudStatesWnd::m_ui_psy_health_show)];

    module(L, "ActorMenu")[def("get_pda_menu", &GetPDAMenu), def("get_actor_menu", &GetActorMenu),
        def("get_menu_mode", &GrabMenuMode), def("get_maingame", &GetMainGameMenu)];
}
