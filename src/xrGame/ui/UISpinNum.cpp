...
UISpinNum::CUISpinNum::CUISpinNum() : m_iVal(0), m_iMin(0), m_iMax(100), m_iStep(1) {}

void UISpinNum::CUISpinNum::SetCurrentOptValue() // opt->current
{
...
void UISpinNum::CUISpinNum::SaveBackUpOptValue() // current->backup
{
    CUIOptionsItem::SaveBackUpOptValue();
    m_opt_backup_value = m_iVal;
}

void UISpinNum::CUISpinNum::SaveOptValue() // current->opt
{
    CUIOptionsItem::SaveOptValue();
    SaveOptIntegerValue(m_iVal);
}

void UISpinNum::CUISpinNum::UndoOptValue() // backup->current
{
...
bool UISpinNum::CUISpinNum::IsChangedOptValue() const // backup!=current
{
    return m_iVal != m_opt_backup_value;
}

void UISpinNum::CUISpinNum::InitSpin(Fvector2 pos, Fvector2 size)
{
    CUICustomSpin::InitSpin(pos, size);
    SetValue(m_iVal);
}

void UISpinNum::CUISpinNum::IncVal()
{
    if (CanPressUp())
        m_iVal += m_iStep;
...
void UISpinNum::CUISpinNum::DecVal()
{
    if (CanPressDown())
        m_iVal -= m_iStep;
...
void UISpinNum::CUISpinNum::OnBtnUpClick()
{
    IncVal();
    CUICustomSpin::OnBtnUpClick();
}

void UISpinNum::CUISpinNum::OnBtnDownClick()
{
    DecVal();
    CUICustomSpin::OnBtnDownClick();
}

void UISpinNum::CUISpinNum::SetValue(int v)
{
    string16 buff;
    m_pLines->SetText(itoa(v, buff, 10));
}

bool UISpinNum::CUISpinNum::CanPressUp() { return m_iVal + m_iStep <= m_iMax; }

bool UISpinNum::CUISpinNum::CanPressDown() { return m_iVal - m_iStep >= m_iMin; }
