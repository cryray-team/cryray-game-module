...
{
    linestdd::StrSize begin, end, comma1_pos, comma2_pos, comma3_pos;

    begin = str.find(linestdd::BEGIN);
    end = str.find(linestdd::END, begin);
    if (begin == linestdd::npos || end == linestdd::npos)
        return m_dwTextColor;
    // try default color
    if (linestdd::npos != str.find("%c[default]", begin, end - begin))
        return m_dwTextColor;
    ...
    if (comma1_pos == linestdd::npos || comma2_pos == linestdd::npos || comma3_pos == linestdd::npos)
        return m_dwTextColor;
...
void CUILines::CutFirstColoredTextEntry(xr_string& entry, u32& color, xr_string& text) const
{
    ...
    linestdd::StrSize begin = text.find(linestdd::BEGIN);
    linestdd::StrSize end = text.find(linestdd::END, begin);
    if (xr_string::npos == end)
        begin = end;
    linestdd::StrSize begin2 = text.find(linestdd::BEGIN, end);
    linestdd::StrSize end2 = text.find(linestdd::END, begin2);
