...
void CUIMapWnd::Init(LPCSTR xml_name, LPCSTR start_from)
{
    ...
    m_GlobalMap = xr_new<UIGlobalMap::CUIGlobalMap>(this);
    ...
    if (pGameIni->section_exist(sect_name.c_str()))
    {
        ...
        for (; it != end; it++)
        {
            ...
            UICustomMap::CUICustomMap*& l = m_GameMaps[map_name];

            l = xr_new<UILevelMap::CUILevelMap>(this);
...
void CUIMapWnd::AddMapToRender(UICustomMap::CUICustomMap* m)
{
    Register(m);
...
void CUIMapWnd::SetTargetMap(const shared_str& name, const Fvector2& pos, bool bZoomIn)
{
    ...
    if (idx != u16(-1))
    {
        UICustomMap::CUICustomMap* lm = GetMapByIdx(idx);
...
void CUIMapWnd::SetTargetMap(UICustomMap::CUICustomMap* m, bool bZoomIn)
{
    m_tgtMap = m;
    Fvector2 pos;
    Frect r = m->BoundRect();
...
void CUIMapWnd::SetTargetMap(UICustomMap::CUICustomMap* m, const Fvector2& pos, bool bZoomIn)
{
    m_tgtMap = m;

    if (m == GlobalMap())
    {
        UIGlobalMap::CUIGlobalMap* gm = GlobalMap();
...
UICustomMap::CUICustomMap* CUIMapWnd::GetMapByIdx(u16 idx)
{
    VERIFY(idx != u16(-1));
...
void CUIMapWnd::ViewActor()
{
    ...
    UICustomMap::CUICustomMap* lm = NULL;
