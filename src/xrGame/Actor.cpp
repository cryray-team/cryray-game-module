...
#include "../xrGameAPI/Effector/EffectorBobbing.h"
...
#include "../xrGameAPI/Actor/ActorNightVision.h"
        ...
        else
        {
            ...
            if (g_Alive())
            {
                ...
                callback(GameObject::eHit)(this->lua_game_object(), HDS.damage(), HDS.direction(),
                    dynamic_cast<const CGameObject*>(HDS.who)->lua_game_object(), HDS.boneID);
            }
            inherited::Hit(&HDS);
        }
...
void CActor::HitSignal(float perc, Fvector& vLocalDir, CObject* who, s16 element)
{
    ...
    // LPCSTR bone_name = dynamic_cast<IKinematics*>(this->Visual())->LL_BoneName_dbg(element);
...
void CActor::Die(CObject* who)
{
        ...
        for (; I <= E; ++I)
        {
            ...
            if (I == inventory().GetActiveSlot())
            {
                if (item_in_slot)
                {
                    if (IsGameTypeSingle())
                    {
                        CGrenade* grenade = dynamic_cast<CGrenade*>(item_in_slot);
                        if (grenade)
                            grenade->DropGrenade();
                        else
            ...
            {
                CCustomOutfit* pOutfit = dynamic_cast<CCustomOutfit*>(item_in_slot);
                if (pOutfit)
                    continue;
            };
            if (item_in_slot)
    ...
    {
        xrAPI.Sound->play_at_pos(sndDie[Random.randI(SND_DIE_COUNT)], this, Position());
...
extern ENGINE_API float g_ironsights_factor;
...
float CActor::currentFOV()
{
        ...
        (!pWeapon->ZoomTexture() || (!pWeapon->IsRotatingToZoom() && pWeapon->ZoomTexture())))
    {
        if (pWeapon->GetZoomFactor() == 0)
            return atan(tan((g_scope_fov * 100.f) * (0.5 * PI / 180)) / g_ironsights_factor) / (0.5 * PI / 180);
        else
...
void CActor::UpdateCL()
{
    if (g_Alive() && Level().CurrentViewEntity() == this)
    {
        if (CurrentGameUI() &&
            (!CurrentGameUI()->TopInputReceiver() ||
                (CurrentGameUI()->TopInputReceiver() && !CurrentGameUI()->TopInputReceiver()->StopAnyMove())) &&
            !m_holder)
        {
    ...
    CHelmet* pHelmet = dynamic_cast<CHelmet*>(inventory().ItemFromSlot(HELMET_SLOT));
    ...
    if (CryRayFlags32.test(RS_DISCORD_RPC_ACTIVE_ANOMALY))
    {
        // God
        ...
        if (!discord_gameinfo.loadscreen && discord_gameinfo.ex_update)
        {
            ...
            {
                snprintf(discord_gameinfo.level_name, 128, xr_ToUTF8(*CStringTable().translate(g_pGameLevel->name()), 128));
                srand(time(0));
                int level_icon_id = CryRayUtils::GetRandomNumber(1, 4);
            ...
            if (ai().script_engine().functor("_g.IsStoryMode", game_mode) && game_mode())
                snprintf(
                    discord_gameinfo.gamemode, 128, xr_ToUTF8(*CStringTable().translate("st_cap_check_story"), 128));

            // Warfare
            else if (ai().script_engine().functor("_g.IsWarfare", game_mode) && game_mode())
                snprintf(
                    discord_gameinfo.gamemode, 128, xr_ToUTF8(*CStringTable().translate("st_cap_check_warfare"), 128));

            // Azazel Mode
            else if (ai().script_engine().functor("_g.IsAzazelMode", game_mode) && game_mode())
            {
                snprintf(discord_gameinfo.gamemode, 128,
                    xr_ToUTF8(*CStringTable().translate("st_cap_check_azazel_mode"), 128));
            ...
            // Survival Mode
                ...
                snprintf(
                    discord_gameinfo.gamemode, 128, xr_ToUTF8(*CStringTable().translate("st_cap_check_survival"), 128));

            // Freeplay Mode
            else
                snprintf(
                    discord_gameinfo.gamemode, 128, xr_ToUTF8(*CStringTable().translate("st_cap_check_freeplay"), 128));
    ...
    // g_pGamePersistent->actor_data.helmet = !GetOutfit()->bIsHelmetAvaliable || inventory().ItemFromSlot(HELMET_SLOT)
    // ? 1 : 0;
...
void CActor::set_safemode(bool status)
{
    if (is_safemode() != status)
    {
        ...
        CWeapon* wep = dynamic_cast<CWeapon*>(inventory().ActiveItem());
        status ? callback(GameObject::eOnWeaponLowered)(wep ? wep->lua_game_object() : nullptr) :
                 callback(GameObject::eOnWeaponRaised)(wep ? wep->lua_game_object() : nullptr);
    }
}
...
void CActor::RPC_UpdateFaction()
{
    ...
    if (ai().script_engine().functor("_g.get_actor_true_community", real_faction))
    {
        if (real_faction)
        {
            ...
            snprintf(discord_gameinfo.faction_name, 128, xr_ToUTF8(*CStringTable().translate(buffer), 128));
        }
    }
}
...
void CActor::RPC_UpdateRank()
{
    ...
    if (ai().script_engine().functor("ranks.get_player_rank_name", actor_rank))
    {
        if (actor_rank)
        {
            ...
            snprintf(discord_gameinfo.rank_name, 128, xr_ToUTF8(*CStringTable().translate(rank_name), 128));
        }
    }
}
                ...
                if (actor_rep)
                {
                    ...
                    if (reputation_name)
                        snprintf(discord_gameinfo.reputation, 128,
                            xr_ToUTF8(*CStringTable().translate(reputation_name), 128));
                }
            }
        }
    }
...
bool CActor::use_HolderEx(CHolderCustom* object, bool bForce)
{
    if (m_holder)
    {
        if (!m_holder->ExitLocked() || bForce)
        {
            if (!object || (m_holder == object))
            {
                ...
                CPhysicsShellHolder* pholder = dynamic_cast<CPhysicsShellHolder*>(go);
                if (pholder)
                {
    ...
    else
    {
        if (object && !object->EnterLocked() || bForce)
        {
            ...
            Center(center);
            if (bForce ||
                object->Use(Device.vCameraPosition, Device.vCameraDirection, center) && object->attach_Actor(this))
            {
                ...
                m_holder = object;
                CObject* oHolder = dynamic_cast<CObject*>(object);
                ...
                CGameObject* go = dynamic_cast<CGameObject*>(object);
                if (go)
...
{
    CHolderCustom* oHolder = dynamic_cast<CHolderCustom*>(object);
    if (!oHolder)
        return;
...
void CActor::RechargeTorchBattery(void) { m_current_torch->Recharge(); }

CTorch* CActor::GetCurrentTorch(void)
{
    if (inventory().ItemFromSlot(TORCH_SLOT))
    {
        CTorch* torch = dynamic_cast<CTorch*>(inventory().ItemFromSlot(TORCH_SLOT));
        if (torch)
...
void CActor::shedule_Update(u32 DT)
{
    ...
    //------------------------------------------------
    {
        ...
        if (CryRayFlags32.test(RS_FEEL_GRENADE))
...
void CActor::renderable_Render()
{
    ...
    CFlashlight* flashlight = dynamic_cast<CFlashlight*>(inventory().ItemFromSlot(DETECTOR_SLOT));
    ...
    if ((cam_active == eacFirstEye && // first eye cam
            /*xrAPI.Render->get_generation() == xrAPI.Render->GENERATION_R2 &&*/ // R2
            xrAPI.Render->active_phase() == 1) // shadow map rendering on R2
...
void CActor::RenderIndicator(Fvector dpos, float r1, float r2, const ui_shader& IndShader)
{
    ...
    xrAPI.UIRender->StartPrimitive(4, IUIRender::ptTriStrip, IUIRender::pttLIT);
    ...
    xrAPI.UIRender->PushPoint(d.x + pos.x, d.y + pos.y, d.z + pos.z, 0xffffffff, 0.f, 1.f);
    xrAPI.UIRender->PushPoint(a.x + pos.x, a.y + pos.y, a.z + pos.z, 0xffffffff, 0.f, 0.f);
    xrAPI.UIRender->PushPoint(c.x + pos.x, c.y + pos.y, c.z + pos.z, 0xffffffff, 1.f, 1.f);
    xrAPI.UIRender->PushPoint(b.x + pos.x, b.y + pos.y, b.z + pos.z, 0xffffffff, 1.f, 0.f);
    ...
    xrAPI.UIRender->CacheSetXformWorld(Fidentity);
    // RCache.set_xform_world		(Fidentity);
    xrAPI.UIRender->SetShader(*IndShader);
...
void CActor::UpdateArtefactsOnBeltAndOutfit()
{
    ...
    for (TIItemContainer::iterator it = inventory().m_belt.begin(); inventory().m_belt.end() != it; ++it)
    {
        ...
        if (artefact)
        {
            ...
            if (Actor()->conditions().GetArtefactReaction() >= 0.6f)
                lua_G_CryRay::UpdateReactionArtefactScript();
        }
    }
    ...
    if (outfit || pHelmet)
    {
        ...
        conditions().ChangeSatiety(
            ...
            f_update_time);
        conditions().ChangeSleep(
            ((outfit ? outfit->m_fSleepRestoreSpeed : 0.f) + (pHelmet ? pHelmet->m_fSleepRestoreSpeed : 0.f)) *
            f_update_time);
        conditions().ChangeThirst(
            ((outfit ? outfit->m_fThirstRestoreSpeed : 0.f) + (pHelmet ? pHelmet->m_fThirstRestoreSpeed : 0.f)) *
            f_update_time);
...
#include "../xrGameAPI/IWP/Actor.inl"
...
CPHDestroyable* CActor::ph_destroyable() { return dynamic_cast<CPHDestroyable*>(character_physics_support()); }
...
void CActor::SwitchNightVision(bool vision_on, bool use_sounds)
{
    ...
    bool bIsActiveNow = false;

    CHelmet* pHelmet = dynamic_cast<CHelmet*>(inventory().ItemFromSlot(HELMET_SLOT));
    if (pHelmet && pHelmet->m_NightVisionSect.size())
    {
        bIsActiveNow = m_bNightVisionOn;
        ...
        if (m_bNightVisionAllow)
        {
            ...
            {
                m_night_vision->Start(pHelmet->cNameSect(), pHelmet->m_NightVisionSect, use_sounds);
            }
        }
        else
        {
    ...
    else
    {
        ...
        if (pOutfit && pOutfit->m_NightVisionSect.size())
        {
            ...
            if (m_bNightVisionAllow)
            {
                if (m_bNightVisionOn && !bIsActiveNow)
                {
                    m_night_vision->Start(pOutfit->cNameSect(), pOutfit->m_NightVisionSect, use_sounds);
                }
            }
            else
            {
    ...
    if (!m_bNightVisionOn && bIsActiveNow)
        m_night_vision->Stop(use_sounds);
...
void CActor::CreateNightVision()
{
    if (!m_night_vision)
        m_night_vision = xr_new<CNightVisionEffector>(cNameSect());
}

void CActor::SetNightVisionStatus(bool val, bool use_sounds)
{
    if (val)
    {
        SwitchNightVision(val, use_sounds);
        return;
    }
...
void CActor::RepackAmmo()
{
    ...
    {
        CWeaponAmmo* pAmmo = dynamic_cast<CWeaponAmmo*>(_pIItem);
    ...
    while (!_ammo.empty())
    {
        ...
        _ammo.erase(
            std::remove_if(_ammo.begin(), _ammo.end(), [asect](CWeaponAmmo* a) { return a->cNameSect() == asect; }),
            _ammo.end());
    }
}