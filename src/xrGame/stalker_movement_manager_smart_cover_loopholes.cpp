...
bool stalker_movement_manager_smart_cover::test_pick(Fvector source, Fvector destination) const
{
    ...
    struct parameters
    {
        ...
        IC parameters(float& range, CAI_Stalker& object) : m_range(&range), m_object(&object) {}
    }; // struct parameters
    ...
    smart_cover::cover const& cover, shared_str const& loophole_id) const
{
    ...
    Loopholes::const_iterator i = std::find_if(loopholes.begin(), loopholes.end(),
        [&loophole_id](smart_cover::loophole* loophole) { return loophole->id()._get() == loophole_id._get(); });
