...
#include "Include/ParticleCustom.h"
...
void CParticlesObject::Init(LPCSTR p_name, IRender_Sector* S, BOOL bAutoRemove)
{
    ...
    {
        // create visual
        renderable.visual = xrAPI.Render->model_CreateParticles(p_name);
...
void CParticlesObject::renderable_Render()
{
    ...
    xrAPI.Render->set_Transform(&renderable.xform);
    xrAPI.Render->add_Visual(renderable.visual);
}
