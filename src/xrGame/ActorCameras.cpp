...
bool CActor::CanUseFreelook()
{
    ...
    if (inventory().ActiveItem())
    {
        ...
        else if (wep)
        {
            ...
            if (state == CWeapon::eFire || state == CWeapon::eFire2 || state == CWeapon::eReload ||
                state == CWeapon::eSwitch || state == CWeapon::eSwitchMode)
                return false;
        }
    }
...
void CActor::cam_Update(float dt, float fFOV)
{
    ...
    fFPCamYawMagnitude = freelook_last_frame ? 0.f :
                                               (1.f - freelook_cam_control) *
            (angle_difference_signed(cam_yaw_prev, cam_yaw_cur) / Device.fTimeDelta); // L+ / R-
    fFPCamPitchMagnitude = freelook_last_frame ? 0.f :
                                                 (1.f - freelook_cam_control) *
            (angle_difference_signed(cam_pitch_prev, cam_pitch_cur) / Device.fTimeDelta); // U+ / D-
