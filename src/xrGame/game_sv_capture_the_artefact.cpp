...
#include "../xrGameAPI\LevelGameDef.h"
...
void game_sv_CaptureTheArtefact::SM_CheckViewSwitching()
{
    ...
    CUIGameCTA* gameCTA = dynamic_cast<CUIGameCTA*>(CurrentGameUI());
    if (gameCTA)
    {
...
void game_sv_CaptureTheArtefact::OnRoundEnd()
{
    ...
    struct spectator_spawner
    {
        ...
        void operator()(IClient* client)
        {
            ...
            if (ps->IsSkip())
                return;
            if (l_pC->owner && dynamic_cast<CActor*>(Level().Objects.net_Find(l_pC->owner->ID)))
            {
...
void game_sv_CaptureTheArtefact::ReSpawnArtefacts()
{
    ...
    for (TeamsMap::iterator ti = teams.begin(); ti != te; ti++)
    {
        ...
        CSE_ALifeItemArtefact* tempSvEntity =
            dynamic_cast<CSE_ALifeItemArtefact*>(spawn_begin(ti->second.artefactName.c_str()));
        ...
        ti->second.artefact =
            dynamic_cast<CSE_ALifeItemArtefact*>(spawn_end(tempSvEntity, m_server->GetServerClient()->ID));
...
void game_sv_CaptureTheArtefact::StartNewRound()
{
    ...
    auto tmp_functor = fastdelegate::MakeDelegate(this, &game_sv_CaptureTheArtefact::PrepareClientForNewRound);
    m_server->ForEachClientDoSender(tmp_functor);
    MoveLifeActors();
