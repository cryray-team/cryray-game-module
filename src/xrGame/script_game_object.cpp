...
BIND_FUNCTION01(
    &object(), CScriptGameObject::ChangePsyHealth, CEntityAlive, conditions().ChangePsyHealth, float, float);
...
BIND_FUNCTION10(
    &object(), CScriptGameObject::GetArtefactReaction, CEntityAlive, conditions().GetArtefactReaction, float, -1);
BIND_FUNCTION01(
    &object(), CScriptGameObject::SetArtefactReaction, CEntityAlive, conditions().ChangeArtefactReaction, float, float);
...
BIND_FUNCTION01(
    &object(), CScriptGameObject::ChangeRadiation, CEntityAlive, conditions().ChangeRadiation, float, float);
...
BIND_FUNCTION01(
    &object(), CScriptGameObject::ChangeMorale, CEntityAlive, conditions().ChangeEntityMorale, float, float);
...
BIND_FUNCTION01(
    &object(), CScriptGameObject::ChangeCircumspection, CEntityAlive, conditions().ChangeCircumspection, float, float);
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
float CScriptGameObject::GetSatiety() const { return Actor()->conditions().GetSatiety(); }

void CScriptGameObject::ChangeSatiety(float fNewValue) { Actor()->conditions().ChangeSatiety(fNewValue); }

float CScriptGameObject::GetThirst() const { return Actor()->conditions().GetThirst(); }

void CScriptGameObject::ChangeThirst(float fNewValue) { Actor()->conditions().ChangeThirst(fNewValue); }

float CScriptGameObject::GetSleep() const { return Actor()->conditions().GetSleep(); }

void CScriptGameObject::ChangeSleep(float fNewValue) { Actor()->conditions().ChangeSleep(fNewValue); }
...
Fvector CScriptGameObject::bone_position(LPCSTR bone_name, bool bHud) const
{
    ...
    CHudItem* itm = dynamic_cast<CHudItem*>(&object());
...
float CScriptGameObject::GetPowerCritical() const
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CSciptEntity : cannot access class member GetPowerCritical!");
        return 0.f;
    }
...
{
    CPda* pda = dynamic_cast<CPda*>(&object());
    if (!pda)
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CSciptEntity : cannot access class member GetPsyFactor!");
        return 0.f;
    }
...
void CScriptGameObject::SetPsyFactor(float val)
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CSciptEntity : cannot access class member SetPsyFactor!");
        return;
    }
...
{
    smart_cover::object* smart_cover_object = dynamic_cast<smart_cover::object*>(&object());
...
{
    CPhantom* phant = dynamic_cast<CPhantom*>(&object());
    if (!phant)
        return;
...
bool CScriptGameObject::Use(CScriptGameObject* obj)
{
    ...
    CActor* actor = dynamic_cast<CActor*>(&obj->object());
    if (!actor)
        return ret;

    CInventoryOwner* pActorInv = dynamic_cast<CInventoryOwner*>(actor);
    if (!pActorInv)
        return ret;
    ...
    CInventoryBox* pBox = dynamic_cast<CInventoryBox*>(&object());
    if (pBox)
    {
    ...
    CInventoryOwner* pOtherOwner = dynamic_cast<CInventoryOwner*>(&object());
    if (pOtherOwner)
    {
