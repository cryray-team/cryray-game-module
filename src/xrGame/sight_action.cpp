    ...
    if (count > 1)
    {
        ...
        if (Device.dwTimeGlobal - previous_position.dwTime < 300)
        {
            if (current_position.dwTime > previous_position.dwTime)
            {
                ...
                const float g_aim_predict_time =
                    pStructCryRay->hard_vis_npc ? ai_aim_predict_time_hard : ai_aim_predict_time_medium;

                float const predict_time = g_aim_predict_time;
