...
void game_sv_GameState::net_Export_State(NET_Packet& P, ClientID to)
{
    ...
    auto pcounter = fastdelegate::MakeDelegate(&tmp_functor, &player_exporter::count_players);

    // fastdelegate::FastDelegate1<IClient*, void> exporter;
    // exporter.bind(&tmp_functor, &player_exporter::export_players);
    auto exporter = fastdelegate::MakeDelegate(&tmp_functor, &player_exporter::export_players);
...
{
    xrGameSpyServer* gs_server = dynamic_cast<xrGameSpyServer*>(m_server);
    R_ASSERT(gs_server);
...
void game_sv_GameState::MapRotation_ListMaps()
{
    ...
    for (u32 i = 0; i < m_pMapRotation_List.size(); i++)
    {
        ...
        if (i == 0)
            Msg(
                "~   %d. %s (%s) (current)", i + 1, st.translate(R.map_name).c_str(), R.map_name.c_str());
        else
...
public:
    ...
    IC bool operator()(IClient* client) const
    {
        if (client == m_exclude)
            return false;
