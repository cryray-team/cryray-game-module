...
void xrServer::client_Destroy(IClient* C)
{
    ...
    IClient* alife_client =
        net_players.FindAndEraseClient(std::bind(std::equal_to<IClient*>(), C, std::placeholders::_1));
    // VERIFY(alife_client);
    if (alife_client)
    {
...
struct PlayerInfoWriter
{
    ...
    {
        xrClientData* tmp_client = dynamic_cast<xrClientData*>(C);
        if (!tmp_client)
            return;
