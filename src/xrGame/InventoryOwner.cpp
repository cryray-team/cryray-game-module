...
BOOL CInventoryOwner::net_Spawn(CSE_Abstract* DC)
{
    ...
    if (IsGameTypeSingle())
    {
        ...
        if (E)
            pTrader = dynamic_cast<CSE_ALifeTraderAbstract*>(E);
        if (!pTrader)
            return FALSE;
...
void CInventoryOwner::StopTalk()
{
    ...
    CUIGameSP* ui_sp = dynamic_cast<CUIGameSP*>(CurrentGameUI());
...
void CInventoryOwner::spawn_supplies()
{
    ...
    VERIFY(game_object);
    if (dynamic_cast<CBaseMonster*>(this))
        return;
...
u16 CInventoryOwner::object_id() const { return dynamic_cast<const CGameObject*>(this)->ID(); }
...
{
    CEntityAlive* EA = dynamic_cast<CEntityAlive*>(this);
    VERIFY(EA);
...
{
    return dynamic_cast<CCustomOutfit*>(inventory().ItemFromSlot(OUTFIT_SLOT));
}
...
void CInventoryOwner::sell_useless_items()
{
    ...
    for (; I != E; ++I)
    {
        ...
        CInventoryItem* item = dynamic_cast<CInventoryItem*>(*I);
...
CInventoryItem* CInventoryOwner::GetCurrentTorchOwner() const { return inventory().ItemFromSlot(TORCH_SLOT); }