...
CGamePersistent::CGamePersistent(void)
{
    ...
    Fvector3* DofValue = Console->GetFVectorPtr("xrRender_dof");
    SetBaseDof(*DofValue);
...
void CGamePersistent::update_logo_intro()
{
    if (m_intro && (false == m_intro->IsActive()))
    {
		...
		Msg("~ MT Log start");
		CryRayAPI.MTLogStart = true;
        pStructCryRay->BLockfpsIntro = FALSE;
...
void CGamePersistent::game_loaded()
{
    ...
    {
        if (g_pGameLevel && g_pGameLevel->bReady && (allow_intro() && CryRayFlags32.test(RS_KEYSTART_PRESS)) &&
...
void CGamePersistent::RenderEditors() { ImGuiCore->ShowEditor(); }

bool CGamePersistent::EditorKeyPress(int key) { return ImGuiCore->EditorKeyPressCallback(key); }

void CGamePersistent::OnFrame()
{
    xrImGui::AddToRender(xrImGui::RenderFunc(this, &CGamePersistent::RenderEditors));
    xrImGui::AddKeyPressCallback(xrImGui::KeyFunc(this, &CGamePersistent::EditorKeyPress));
...
{
    string512 buff;
    constexpr const char* dots = ""; // if title is empty don't insert dots
...
void CGamePersistent::GetCurrentDof(Fvector3& dof) { dof = m_dof[eDofCurrent]; }
