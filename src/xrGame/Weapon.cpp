...
void CWeapon::Load(LPCSTR section)
{
    ...
    iMagazineSize2 = 1;
    iMagazineSize_ui = READ_IF_EXISTS(pSettings, r_s32, section, "ammo_mag_size_ui", iMagazineSize);
...
bool CWeapon::NeedBlendAnm()
{
    ...
    if (IsZoomed() && CryRayFlags32.test(RS_AIM_SWAY))
        return true;
...
bool CWeapon::AllowBore() { return !Actor()->is_safemode(); }
            ...
            if (!m_zoom_params.m_pNight_vision->IsActive())
            {
                ...
                if (!pA->GetNightVisionStatus())
                    m_zoom_params.m_pNight_vision->Start(
                        m_zoom_params.m_sUseZoomPostprocess, m_zoom_params.m_sUseZoomPostprocess, false);
            }
        }
        else
        {
            if (m_zoom_params.m_pNight_vision)
            {
                if (m_zoom_params.m_pNight_vision->IsActive())
                {
                    m_zoom_params.m_pNight_vision->Stop(false);
                }
            }
        }
    }
...
int CWeapon::GetAmmoCount_forType(shared_str const& ammo_type) const
{
    if (use_ammo_on_belt)
    {
        ...
        for (; itb != ite; ++itb)
        {
            CWeaponAmmo* pAmmo = dynamic_cast<CWeaponAmmo*>(*itb);
        ...
        auto entity_alive = dynamic_cast<CEntityAlive*>(parent);
        ...
        if (pA)
        {
            if (pA && !m_zoom_params.m_pNight_vision)
            {
                m_zoom_params.m_pNight_vision =
                    xr_new<CNightVisionEffector>(m_zoom_params.m_sUseZoomPostprocess);
            }
        }
    }
...
void CWeapon::SwitchState(u32 S)
{
    ...
    {
        // !!! Just single entry for given state !!!f
        NET_Packet P;
        CHudItem::object().u_EventGen(P, GE_WPN_STATE_CHANGE, CHudItem::object().ID());
        P.w_u8(u8(S));
        P.w_u8(u8(m_sub_state));
...
{
    // Alundaio
    const CInventoryOwner* io = dynamic_cast<const CInventoryOwner*>(H_Parent());
    if (!io)
        return false;
...
void CWeapon::UpdateHudAdditional(Fmatrix& trans)
{
    ...
    */
    ...
    //============= Поворот ствола во время аима =============//
    {
        ...
        if (idx == 4 || last_idx == 4)
            ...
            factor = Device.fTimeDelta /
                (m_zoom_params.m_fZoomRotateTime * cur_silencer_koef.zoom_rotate_time *
    ...
    fAvgTimeDelta = _inertion_vector(fAvgTimeDelta, Device.fTimeDelta, 0.8f);
...
void CWeapon::OnStateSwitch(u32 S, u32 oldState)
{
    ...
    if (GetState() == eReload)
    {
        if (m_ammoElapsed.type1 == 0) // Swartz: re-written to use reload empty DOF
        {
            if (H_Parent() == Level().CurrentEntity() && !fsimilar(m_zoom_params.m_ReloadEmptyDof.w, -1.0f))
            {
                CActor* current_actor = dynamic_cast<CActor*>(H_Parent());
                if (current_actor)
    ...
    if (ParentIsActor() && dynamic_cast<CActor*>(H_Parent())->inventory().ActiveItem() == this && GetState() != eIdle)
        Actor()->set_safemode(false);
}
...
void CWeapon::zoom_sound_inс()
{
    sndZoom.play(Actor(), sm_2D);
    sndZoom.create("scopes\\snd_zoom_inc", st_Effect, SOUND_TYPE_ITEM_USING);
};
...
void CWeapon::zoom_sound_dec()
{
    sndZoom.play(Actor(), sm_2D);
    sndZoom.create("scopes\\snd_zoom_dec", st_Effect, SOUND_TYPE_ITEM_USING);
};
