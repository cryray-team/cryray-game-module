...
smart_cover::cover::cover(smart_cover::object const& object, DescriptionPtr description, bool const is_combat_cover,
...
u32 const& smart_cover::cover::action_level_vertex_id(smart_cover::loophole const& loophole, shared_str const& action_id) const
{
    Vertices::const_iterator found = std::find_if(
        m_vertices.begin(), m_vertices.end(), [&loophole](smart_cover::cover::Vertex const& vertex) -> bool {
            return vertex.first->id().equal(loophole.id());
        });
    VERIFY(found != m_vertices.end());

    loophole_data::ActionVertices::const_iterator found2 = std::find_if(found->second.m_action_vertices.begin(), found->second.m_action_vertices.end(),
            [&action_id](smart_cover::loophole_data::Action const& action) { return action_id.equal(action.first); });
...
smart_cover::loophole* smart_cover::cover::best_loophole(
...
void smart_cover::cover::evaluate_loophole(Fvector const& position, smart_cover::loophole*& source,
...
void smart_cover::cover::evaluate_loophole_for_default_usage(
...
struct loophole_predicate
{
    ...
    IC bool operator()(smart_cover::cover::Vertex const& vertex) const { return (vertex.first == m_loophole); }
};
...
bool smart_cover::cover::in_min_acceptable_range(
