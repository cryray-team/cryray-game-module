    ...
    case kQUICK_USE_4: {
        ...
        if (item_name.size())
        {
            ...
            if (itm && ai().script_engine().functor("_G.CInventory__eat", funct))
            {
                ...
                {
                    auto l_InfoUseItem = [&](bool bandage, bool debug) -> void 
                    {
                        if (!bandage)
                            inventory().Eat(itm);

                        if (debug)
                            Msg("~ [%s]-> item: '%s'", __FUNCTION__, item_name.c_str());

                        SDrawStaticStruct* _s = CurrentGameUI()->AddCustomStatic("item_used", true);
                        string1024 str;
                        strconcat(sizeof(str), str,
                            *CStringTable().translate(bandage ? "st_item_not_used" : "st_item_used"), ": ",
                            itm->NameItem());
                    ...
                    //if ((itm && BandageUse < fis_zero(BandageUse, EPS)) && (strcmp(item_name.c_str(), "bandage") == 0))
                    //{
