...
void action::load_animations(luabind::object const& table)
{
    ...
    for (; I != E; ++I)
    {
        ...
        MonsterSpace::EBodyState body_state =
            (MonsterSpace::EBodyState)smart_cover::detail::parse_int(tmp, "body_state");
        MonsterSpace::EMovementType movement_type =
            (MonsterSpace::EMovementType)smart_cover::detail::parse_int(tmp, "movement_type");
...
animation_action const& action::animation(MonsterSpace::EBodyState const& target_body_state) const
{
    ...
    if (found == m_animations.end())
    {
        ...
        log_cryray_engine::Msg(
            "! There is no animation which can transfer bot to body_state [%i], selecting random transition",
            target_body_state);
