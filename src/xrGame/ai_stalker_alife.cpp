...
namespace ai_stalker_alife
{
    static const int MaxAmmoAttachCount = 1;
    static const int EnoughAmmoBoxCount = 1;
} // namespace
...
void CAI_Stalker::attach_available_ammo(CWeapon* weapon)
{
    ...
    for (; I != E; ++I)
    {
        ...
        ++count;
        if (count >= ai_stalker_alife::MaxAmmoAttachCount)
            break;
    }
}
...
bool CAI_Stalker::enough_ammo(const CWeapon* new_weapon) const
{
    ...
    for (; I != E; ++I)
    {
        ...
        ++ammo_box_count;
        if (ammo_box_count >= ai_stalker_alife::EnoughAmmoBoxCount)
            return (true);
    }
