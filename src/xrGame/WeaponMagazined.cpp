...
void CWeaponMagazined::Load(LPCSTR section)
{
    ...
    if (WeaponSoundExist(section, "snd_mag_shot"))
...
void CWeaponMagazined::FireStart()
{
    ...
    else
    {
        // misfire
        // Alundaio
        CGameObject* object = dynamic_cast<CGameObject*>(H_Parent());
        if (object)
...
bool CWeaponMagazined::TryToGetAmmo(u32 id)
{
    if (use_ammo_on_belt)
    {
        if (dynamic_cast<CActor*>(H_Parent()) != NULL)
        {
            m_pCurrentAmmo = dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAmmoOnBelt(m_ammoTypes[id].c_str()));
        }
        else
        {
            m_pCurrentAmmo = dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAny(m_ammoTypes[id].c_str()));
        }
...
bool CWeaponMagazined::TryReload()
{
    if (use_ammo_on_belt)
    {
        if (m_pInventory)
        {
            ...
            {
                if (TryToGetAmmo(m_ammoType.type1) || unlimited_ammo())
                {
                    SetPending(true);
                    SwitchState(eReload);
                    return true;
    ...
    else
    {
        if (m_pInventory)
        {
            ...
            m_pCurrentAmmo = dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAny(m_ammoTypes[m_ammoType.type1].c_str()));
            ...
            if (m_pCurrentAmmo || unlimited_ammo())
            {
                ...
                for (u8 i = 0; i < u8(m_ammoTypes.size()); ++i)
                {
                    m_pCurrentAmmo = dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAny(m_ammoTypes[i].c_str()));
                    if (m_pCurrentAmmo)
                    {
                        m_set_next_ammoType_on_reload = i;
                        SetPending(true);
...
bool CWeaponMagazined::IsAmmoAvailable()
{
    ...
    {
        if (ParentIsActor())
            m_pCurrentAmmo = dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAmmoOnBelt(m_ammoTypes[m_ammoType.type1].c_str()));
        else 
            m_pCurrentAmmo = dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAny(m_ammoTypes[m_ammoType.type1].c_str()));
        ...
        else
        {
            for (u32 i = 0; i < m_ammoTypes.size(); ++i)
                ...
                if (ParentIsActor())
                    m_pCurrentAmmo = dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAmmoOnBelt(m_ammoTypes[i].c_str()));
                else
                    m_pCurrentAmmo = dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAny(m_ammoTypes[i].c_str()));
        }
    ...
    {
        if (dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAny(m_ammoTypes[m_ammoType.type1].c_str())))
            return true;
        else
            for (u32 i = 0; i < m_ammoTypes.size(); ++i)
            {
                if (dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAny(m_ammoTypes[i].c_str())))
                    return true;
            }
...
void CWeaponMagazined::ReloadMagazine()
{
    if (use_ammo_on_belt)
    {
        ...
        if (!unlimited_ammo())
        {
            ...
            {
                m_pCurrentAmmo =
                    dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAmmoOnBelt(m_ammoTypes[m_ammoType.type1].c_str()));
            }
            else
            {
                m_pCurrentAmmo =
                    dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAny(m_ammoTypes[m_ammoType.type1].c_str()));
            }
            ...
            if (ParentIsActor())
            {
                m_pCurrentAmmo = dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAmmoOnBelt(tmp_sect_name_belt));
            }
            else
            {
                m_pCurrentAmmo = dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAny(tmp_sect_name));
            }
            ...
            if (!m_pCurrentAmmo && !m_bLockType)
            {
                for (u8 i = 0; i < u8(m_ammoTypes.size()); ++i)
                {
                    ...
                    {
                        m_pCurrentAmmo =
                            dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAmmoOnBelt(m_ammoTypes[i].c_str()));
                    }
                    else
                    {
                        m_pCurrentAmmo = dynamic_cast<CWeaponAmmo*>(m_pInventory->GetAny(m_ammoTypes[i].c_str()));
...
void CWeaponMagazined::OnStateSwitch(u32 S, u32 oldState)
{
    ...
    CInventoryOwner* owner = dynamic_cast<CInventoryOwner*>(this->H_Parent());
    switch (S)
    {
...
void CWeaponMagazined::state_Fire(float dt)
{
    if (m_ammoElapsed.type1 > 0)
    {
        ...
        if (!H_Parent())
            return;
        if (dynamic_cast<CMPPlayersBag*>(H_Parent()) != NULL)
        {
        ...
        if (NULL == io->inventory().ActiveItem())
        {
            ...
            StopShooting();
            return;
            // Alundaio: This is not supposed to happen but it does. GSC was aware but why no return here? Known to
            // cause crash on game load if npc immediatly enters combat.
        }
...
void CWeaponMagazined::OnShot()
{
    ...
    float factor = fAmmoElapsed / fmaxMagazineSize_;
    ...
    CGameObject* object = dynamic_cast<CGameObject*>(H_Parent());
    if (object)
        object->callback(GameObject::eOnWeaponFired)(object->lua_game_object(), this->lua_game_object(),
            m_ammoElapsed.type1, m_ammoElapsed.type2, m_ammoType.type1, m_ammoType.type2);
...
void CWeaponMagazined::OnEmptyClick()
{
    ...
    if (Actor() && GetHUDmode() && IsMisfire())
        lua_G_CryRay::WeaponJammedScript();
}
...
void CWeaponMagazined::PlayReloadSound()
{
    if (m_sounds_enabled)
    {
        if (bMisfire && m_ammoElapsed.type1)
        {
            if (ParentIsActor())
            {
                if (m_sounds.FindSoundItem("sndReloadMisfireActor", false))
...
void CWeaponMagazined::PlayAnimFireModeSwitch()
{
    ...
    {
        SetPending(TRUE);
        m_ammoElapsed.type1 == 0 && HudAnimationExist("anm_switch_mode_empty") ?
...
{
    if (m_ammoElapsed.type1 == 0 && HudAnimationExist("anm_bore_empty"))
    {
...
{
    m_ammoElapsed.type1 == 0 && HudAnimationExist("anm_idle_sprint_empty") ?
...
void CWeaponMagazined::PlayAnimIdleMoving()
{
    ...
    m_ammoElapsed.type1 == 0 && HudAnimationExist("anm_idle_moving_empty") ?
...
void CWeaponMagazined::PlayAnimShow()
{
    ...
    m_ammoElapsed.type1 == 0 && HudAnimationExist("anm_show_empty") ?
...
void CWeaponMagazined::PlayAnimHide()
{
    ...
    m_ammoElapsed.type1 == 0 && HudAnimationExist("anm_hide_empty") ?
...
void CWeaponMagazined::PlayAnimReload()
{
    ...
    if (bMisfire && m_ammoElapsed.type1 != 0)
    {
        // Msg("AVO: ------ MISFIRE");
...
{
    m_ammoElapsed.type1 == 0 && HudAnimationExist("anm_idle_aim_empty") ?
...
void CWeaponMagazined::PlayAnimIdle()
{
    ...
    m_ammoElapsed.type1 == 0 && HudAnimationExist("anm_idle_empty") ?
...
void CWeaponMagazined::PlayAnimShoot()
{
    ...
    if (m_ammoElapsed.type1 > 1 || !HudAnimationExist("anm_shot_l"))
    {
...
void CWeaponMagazined::OnZoomOut()
{
    ...
    if (pActor)
    {
        auto S = dynamic_cast<CEffectorZoomInertion*>(pActor->Cameras().GetCamEffector(eCEZoom));
...
bool CWeaponMagazined::install_upgrade_impl(LPCSTR section, bool test)
{
    ...
    // fov for zoom mode
    result |=
        process_if_exists(section, "scope_zoom_factor", &CInifile::r_float, m_zoom_params.m_fBaseZoomFactor, test);
