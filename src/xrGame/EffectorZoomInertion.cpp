...
#define CAMERA_MOVE_DELTA_TIME                                                                                         \
    (0.150f) // delay to react when camera stopped (to compensate for inconsistent camera angular distance between
             // frames on high FPS)
...
template <class T>
IC T LoadParamInternal(
    LPCSTR section, LPCSTR prefix, LPCSTR param, T (CInifile::*method)(LPCSTR, LPCSTR) const, T defVal)
{
...
bool CEffectorZoomInertion::UpdateCameraMoved(const Fvector& camDir, bool& justStopped)
{
    ...
    else
    {
        ...
        if (cameraSpeed > m_fCameraSpeedThreshold)
        {
            ...
            if (!m_bCameraMoving)
            {
                // Msg("EZI: Camera started moving with speed %.3f (max %.3f), dist: %.4f, time: %.4f, dir:
                // (%.4f,%.4f,%.4f)->(%.4f,%.4f,%.4f)",
                //	cameraSpeed, m_fCameraSpeedThreshold, dist, m_fTimeCameraMove, lastDir.x, lastDir.y, lastDir.z,
        ...
        {
            // Msg("EZI: Camera stopped moving: speed %.3f, dist: %.4f, fDelta: %.4f, dir:
            // (%.4f,%.4f,%.4f)->(%.4f,%.4f,%.4f)", 	cameraSpeed, dist, m_fTimeCameraMove, lastDir.x, lastDir.y,
            //lastDir.z, curDir.x, curDir.y, curDir.z);
...
BOOL CEffectorZoomInertion::ProcessCam(SCamEffectorInfo& info)
{
    if (!m_enabled)
    {
        ...
        {
            m_vCurrentPoint.lerp(
                m_vLastPoint, m_vTargetPoint, SmoothStep(0.f, 1.f, float(m_dwTimePassed) / m_dwCenterDeltaTime));
