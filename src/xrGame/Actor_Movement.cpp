        ...
        if (mstate_real & mcFall)
        {
            if (character_physics_support()->movement()->GetContactSpeed() > 4.f)
            {
                if (fis_zero(character_physics_support()->movement()->gcontact_HealthLost))
                {
                    ...
                    PlayMovementAnm("landing", false, true);
                }
                else
                {
                    m_fLandingTime = s_fLandingTime2;
                    mstate_real |= mcLanding2;
                    PlayMovementAnm("landing2", false, true);
                }
            }

            shared_str cur_name = sCurrentStepMaterial;

            if (cur_name != "asphalt")
                cur_name = "default";
            string_path sound_name;

            strconcat(sizeof(sound_name), sound_name, "actor\\jump_sounds\\land_", cur_name.c_str());

            m_actor_snd[0].destroy();
            m_actor_snd[0].create(sound_name, st_Effect, SOUND_TYPE_IDLE);
            m_actor_snd[0].play(this, sm_2D);

            float volume = m_actor_snd[0].get_params()->volume *
                (character_physics_support()->movement()->GetContactSpeed() / 5.0f);
            clamp(volume, 0.0f, 15.0f);

            m_actor_snd[0].set_volume(volume);
            m_actor_snd[0].set_frequency(Random.randF(0.8f, 1.2f));
...
void CActor::g_cl_CheckControls(u32 mstate_wf, Fvector& vControlAccel, float& Jump, float dt)
{
    ...
    if (curr_env == CPHMovementControl::peOnGround || curr_env == CPHMovementControl::peAtWall)
    {
        ...
        if (CanJump() && (mstate_wf & mcJump))
        {
            ...
            PlayMovementAnm("jump", false, true);

            string_path sound_name;
            strconcat(sizeof(sound_name), sound_name, "actor\\jump_sounds\\jump_", sCurrentStepMaterial.c_str());

            m_actor_snd[1].destroy();
            m_actor_snd[1].create(sound_name, st_Effect, SOUND_TYPE_IDLE);
            m_actor_snd[1].play(this, sm_2D);
            m_actor_snd[1].set_frequency(Random.randF(0.8f, 1.2f));
...
bool CActor::CanJump()
{
    bool can_Jump = !conditions().IsCantSprint() && !character_physics_support()->movement()->PHCapture() &&
        ((mstate_real & mcJump) == 0) && (m_fJumpTime <= 0.f) && (!m_hit_slowmo_jump || fis_zero(m_hit_slowmo)) &&
...
void CActor::OnActorFootstep(shared_str material)
{
    std::vector<std::string> mats = {
        "wood", "earth", "dirt", "metal", "water", "tin", "grass", "gravel", "asphalt", "tile", "concrete", "shifer"};
    sCurrentStepMaterial = "default";
    for (int i = 0; i < mats.size(); i++)
    {
        std::string needed_mat = mats[i];
        std::string cur_mat = material.c_str();
        if (cur_mat.find(needed_mat) != -1)
            sCurrentStepMaterial = mats[i].c_str();
    }

    CCustomOutfit* outfit = GetOutfit();
    if (outfit && outfit->bIsExo)
    {
        u8 rand = Random.randI(1, 5);
        string16 srand;
        _itoa_s(rand, srand, 10);
        string_path sound_name;
        strconcat(sizeof(sound_name), sound_name, "exo\\exo_step", srand);
        m_actor_snd[2].create(sound_name, st_Effect, SOUND_TYPE_IDLE);
        m_actor_snd[2].play_no_feedback(this, sm_2D);
        m_actor_snd[2].set_volume(Random.randF(0.8f, 1.2f));
        m_actor_snd[2].set_frequency(Random.randF(0.8f, 1.2f));
    }
}