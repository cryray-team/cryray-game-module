...
{
    xrAPI.Render->set_Transform(&m_item_transform);
    xrAPI.Render->add_Visual(m_model->dcast_RenderVisual());
...
void player_hud::render_hud()
{
    ...
    xrAPI.Render->set_Transform(&m_transform);
    xrAPI.Render->add_Visual(m_model->dcast_RenderVisual());
    xrAPI.Render->set_Transform(&m_transform_2);
    xrAPI.Render->add_Visual(m_model_2->dcast_RenderVisual());
    ...
    {
        xrAPI.Render->set_Transform(&m_item_pos);
        xrAPI.Render->add_Visual(script_anim_item_model->dcast_RenderVisual());
    }
}
...
#include "Inventory.h"
extern ENGINE_API float g_freelook_z_offset;
...
u32 player_hud::script_anim_play(u8 hand, LPCSTR section, LPCSTR anm_name, bool bMixIn, float speed)
{
    ...
    if (pSettings->line_exist(section, "item_visual"))
    {
        script_anim_item_model =
            xrAPI.Render->model_Create(pSettings->r_string(section, "item_visual"))->dcast_PKinematicsAnimated();
