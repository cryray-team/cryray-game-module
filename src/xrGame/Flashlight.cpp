...
BOOL CFlashlight::net_Spawn(CSE_Abstract* DC)
{
    ...
    Fcolor clr = pSettings->r_fcolor(m_light_section, "color_r2");
    def_color = clr;
    fBrightness = clr.intensity();
    float range = pSettings->r_float(m_light_section, "range_r2");
    light_render->set_color(clr);
    light_render->set_range(range);
    light_render->set_hud_mode(true);

    Fcolor clr_o = pSettings->r_fcolor(m_light_section, "omni_color_r2");
    float range_o = pSettings->r_float(m_light_section, "omni_range_r2");
    ...
    light_render->set_volumetric_quality(
        READ_IF_EXISTS(pSettings, r_float, m_light_section, "volumetric_quality", 1.f));
    light_render->set_volumetric_intensity(
        READ_IF_EXISTS(pSettings, r_float, m_light_section, "volumetric_intensity", 1.f));
    light_render->set_volumetric_distance(
        READ_IF_EXISTS(pSettings, r_float, m_light_section, "volumetric_distance", 1.f));
...
void CFlashlight::UpdateCL()
{
    ...
    {
        dir.setHP(
            -angle_normalize_signed(Actor()->old_torso_yaw), dir.getP() > 0.f ? dir.getP() * .6f : dir.getP() * .8f);
...
IC bool CFlashlight::can_use_dynamic_lights()
{
    if (!H_Parent())
        return (true);
...
bool CFlashlight::torch_active() const { return m_bWorking; }
