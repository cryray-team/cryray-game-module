...
namespace NSearcherClientByName
{
...
void game_sv_mp::OnVoteStart(LPCSTR VoteCommand, ClientID sender)
{
    ...
    if (m_bVotingReal)
    {
        ...
        {
            NSearcherClientByName::SearcherClientByName tmp_predicate(CommandParams);
        ...
        else if (!stricmp(votecommands[i].name, "ban"))
        {
            ...
            // if (ban_time)
            //{
            NSearcherClientByName::SearcherClientByName tmp_predicate(tmp_victim_name);
            IClient* tmp_client = m_server->FindClient(tmp_predicate);
            if (tmp_client)
            {
