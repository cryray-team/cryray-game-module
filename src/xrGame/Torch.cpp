...
shared_str CTorch::m_nv_effect_s = "";

CTorch::CTorch(void) : m_current_battery_state(0)
{
    light_render = xrAPI.Render->light_create();
    light_render->set_type(IRender_Light::SPOT);
    light_render->set_shadow(true);
    light_omni = xrAPI.Render->light_create();
    light_omni->set_type(IRender_Light::POINT);
    light_omni->set_shadow(false);

    m_switched_on = false;
    glow_render = xrAPI.Render->glow_create();
    lanim = 0;
    fBrightness = 1.f;
...
void CTorch::OnMoveToSlot(const SInvItemPlace& prev)
{
    ...
    if (owner && !owner->attached(this))
    {
        ...
        CryRayAPI.bTotchOnSlot = true;
        m_nv_effect_s = m_nv_effect;
        Msg("! Section: %s", m_nv_effect_s.c_str());
    }
}
...
void CTorch::Load(LPCSTR section)
{
    ...
    m_torch_inertion_speed_max =
        READ_IF_EXISTS(pSettings, r_float, section, "torch_inertion_speed_max", TORCH_INERTION_SPEED_MAX);
    m_torch_inertion_speed_min =
        READ_IF_EXISTS(pSettings, r_float, section, "torch_inertion_speed_min", TORCH_INERTION_SPEED_MIN);
    m_torch_inertion_clamp = READ_IF_EXISTS(pSettings, r_float, section, "torch_inertion_clamp", TORCH_INERTION_CLAMP);

    m_battery_duration = READ_IF_EXISTS(pSettings, r_u32, section, "battery_duration", 70000);
    ...
    m_bUseInertion = READ_IF_EXISTS(pSettings, r_bool, section, "torch_inertion", true);

    m_nv_effect = READ_IF_EXISTS(pSettings, r_string, section, "nv_effect", "");
}
...
{
    Msg("SetBatteryStatus = [%d], [%d], [%d], [%d], [%d], [%f]", m_current_battery_state, val, m_battery_duration,
        m_current_battery_state - val, (m_current_battery_state - val) / m_battery_duration * 100,
        (m_current_battery_state - val) / m_battery_duration);
    m_battery_state = val;
...
void CTorch::Switch(bool light_on)
{
    ...
    m_actor_item = (pActor) ? true : false;
    if (light_on && m_battery_state < 0 && m_actor_item)
    {
        light_on = false;

        SDrawStaticStruct* _s = CurrentGameUI()->AddCustomStatic("torch_battery_low", true);
    ...
    if (m_switched_on && m_actor_item)
        HUD_SOUND_ITEM::PlaySound(m_FlashlightSwitchSnd, pActor->Position(), pActor, true, false);
}
...
void CTorch::LoadLightParams()
{
    ...
    Fcolor clr = pSettings->r_fcolor(light_definition.c_str(), "color_r2");
    def_clr = clr;
    fBrightness = clr.intensity();
    m_RangeMax = pSettings->r_float(light_definition.c_str(), "range_r2");
    m_RangeCurve = READ_IF_EXISTS(pSettings, r_float, light_definition.c_str(), "range_curve", m_RangeMax + 5.f);
    light_render->set_color(clr);
    light_render->set_range(m_RangeMax);

    Fcolor clr_o = pSettings->r_fcolor(light_definition.c_str(), "omni_color_r2");
    float range_o = pSettings->r_float(light_definition.c_str(), "omni_range_r2");
    ...
    light_render->set_volumetric(pSettings->r_bool(light_definition.c_str(), "volumetric"));
    light_render->set_volumetric(pSettings->r_float(light_definition.c_str(), "volumetric_distance"));
    light_render->set_volumetric(pSettings->r_float(light_definition.c_str(), "volumetric_intensity"));
    light_render->set_volumetric(pSettings->r_float(light_definition.c_str(), "volumetric_quality"));
...
u32 CTorch::GetBatteryLifetime() const { return m_battery_duration; }

u32 CTorch::GetBatteryStatus() const { return m_battery_state; }
...
void CTorch::afterAttach() { LoadLightParams(); }
