...
//(>1.f, 1.f -means no dist change of exp effect)	on the dist of m_fBlastRadius exp. wave effect in
...
void CExplosive::Explode()
{
...
#endif
    g_pGamePersistent->GrassBendersAddExplosion(cast_game_object()->ID(), pos, Fvector().set(0, -99, 0), 1.33f,
        ps_ssfx_int_grass_params_2.y, ps_ssfx_int_grass_params_2.x, m_fBlastRadius * 2.0f);
    ...
    if (Initiator() != ALife::_OBJECT_ID(-1))
    {
        ...
        if (chance < 75)
        {
            ...
            // Ваш код, который должен выполниться с вероятностью 75%
        }
    }
