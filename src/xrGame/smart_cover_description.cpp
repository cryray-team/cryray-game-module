...
{
    return (transform_vertex(smart_cover::detail::parse_string(table, identifier), in));
}
} // namespace smart_cover
...
smart_cover::loophole const* smart_cover::description::loophole(shared_str const& loophole_id) const
{
    Loopholes::const_iterator found = std::find_if(
        m_loopholes.begin(), m_loopholes.end(), [loophole_id](smart_cover::loophole const* loophole) -> bool {
