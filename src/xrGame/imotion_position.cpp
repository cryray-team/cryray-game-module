...
{
    save_max(CryRayStaticValues::depth, c.geom.depth);
#ifdef DEBUG
...
float imotion_position::motion_collide(float dt, IKinematicsAnimated& KA)
{
    ...
    if (CryRayStaticValues::depth > depth_resolve)
    {
#ifdef DEBUG
        if (dbg_imotion_collide_debug)
        {
        ...
        float depth0 = CryRayStaticValues::depth;
        ...
        {
            interactive_motion_diagnostic(
                make_string(" motion_collide collided1: deppth= %f", CryRayStaticValues::depth).c_str());
            DBG_OpenCashedDraw();
        ...
        if (CryRayStaticValues::depth > depth0)
        {
            interactive_motion_diagnostic(make_string(
                "motion_collide 1: stoped: colide: %s, depth %f", collide_diag().c_str(), CryRayStaticValues::depth)
                                              .c_str());
            flags.set(fl_switch_dm_toragdoll, TRUE);
        }
        else
        {
            depth0 = CryRayStaticValues::depth;
            advance_time += collide_animation(collide_adwance_delta, KA);
            ...
            {
                interactive_motion_diagnostic(
                    make_string(" motion_collide collided2: deppth= %f", CryRayStaticValues::depth).c_str());
                DBG_OpenCashedDraw();
                shell->dbg_draw_geometry(0.02, D3DCOLOR_ARGB(255, 0, 255, 0));
                DBG_ClosedCashedDraw(50000);
            }
            ...
            {
                interactive_motion_diagnostic(make_string(
                    "motion_collide 2: stoped: colide: %s, depth %f", collide_diag().c_str(), CryRayStaticValues::depth)
                        .c_str());
                flags.set(fl_switch_dm_toragdoll, TRUE);
            }
        }
        ...
        if (dbg_imotion_collide_debug)
        {
            ...
            shell->CollideAll();
            interactive_motion_diagnostic(
                make_string(" motion_collide restore: %f ", CryRayStaticValues::depth).c_str());
            DBG_OpenCashedDraw();
