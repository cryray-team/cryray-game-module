...
void CreateUIGeom() { xrAPI.UIRender->CreateUIGeom(); }

void DestroyUIGeom() { xrAPI.UIRender->DestroyUIGeom(); }
...
void CUIStaticItem::RenderInternal(const Fvector2& in_pos)
{
    ...
    Fvector2 ts;
    xrAPI.UIRender->GetActiveTextureResolution(ts);
    ...
    if (R && R->size())
    {
        ...
        {
            xrAPI.UIRender->PushPoint((*R)[0 + 0].pt.x, (*R)[0 + 0].pt.y, 0, dwColor, (*R)[0 + 0].uv.x, (*R)[0 + 0].uv.y);
            xrAPI.UIRender->PushPoint((*R)[k + 1].pt.x, (*R)[k + 1].pt.y, 0, dwColor, (*R)[k + 1].uv.x, (*R)[k + 1].uv.y);
            xrAPI.UIRender->PushPoint((*R)[k + 2].pt.x, (*R)[k + 2].pt.y, 0, dwColor, (*R)[k + 2].uv.x, (*R)[k + 2].uv.y);
        }
    }
}
...
void CUIStaticItem::Render()
{
    ...
    xrAPI.UIRender->StartPrimitive(8, IUIRender::ptTriList, UI().m_currentPointType);
    RenderInternal(vPos);
...
void CUIStaticItem::Render(float angle)
{
    ...
    xrAPI.UIRender->SetShader(*hShader);
    xrAPI.UIRender->StartPrimitive(32, IUIRender::ptTriList, UI().m_currentPointType);
