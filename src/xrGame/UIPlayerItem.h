...
private:
    ...
    ClientID myClientId;
    UIPlayerItem();
    IC s32 CalculateCheckPoints(game_PlayerState const* ps) const;

    void InitTextParams(CUIXml& uiXml);
    void InitIconParams(CUIXml& uiXml);

    IC void UpdateTextParams(game_PlayerState const* ps);
    IC void UpdateIconParams(game_PlayerState const* ps);

    IC void GetTextParamValue(game_PlayerState const* ps, shared_str const& param_name, buffer_vector<char>& dest);

    IC void GetIconParamValue(game_PlayerState const* ps, shared_str const& param_name, buffer_vector<char>& dest);
