...
ICF static BOOL HudTarget_pick_trace_callback(collide::rq_result& result, LPVOID params)
{
...
void CHUDTarget::CursorOnFrame()
{
    ...
    if (Level().CurrentEntity())
    {
        ...
        if (Level().ObjectSpace.RayQuery(RQR, RD, HudTarget_pick_trace_callback, &PP, NULL, Level().CurrentEntity()))
...
void CHUDTarget::Render()
{
            ...
            if (IsGameTypeSingle())
            {
                ...
                else if (E && E->g_Alive() && !E->cast_base_monster())
                {
                    ...
                    if (our_inv_owner && others_inv_owner)
                    {
                        ...
                        {
                        case ALife::eRelationTypeEnemy: C = HudTarget::C_ON_ENEMY; break;
                        case ALife::eRelationTypeNeutral: C = HudTarget::C_ON_NEUTRAL; break;
                        case ALife::eRelationTypeFriend: C = HudTarget::C_ON_FRIEND; break;
                        }
    ...
    if (!m_bShowCrosshair)
    {
        xrAPI.UIRender->StartPrimitive(6, IUIRender::ptTriList, UI().m_currentPointType);
        ...
        //	Tri 1
        xrAPI.UIRender->PushPoint(cx - size_x, cy + size_y, 0, C, 0, 1);
        xrAPI.UIRender->PushPoint(cx - size_x, cy - size_y, 0, C, 0, 0);
        xrAPI.UIRender->PushPoint(cx + size_x, cy + size_y, 0, C, 1, 1);
        //	Tri 2
        xrAPI.UIRender->PushPoint(cx + size_x, cy + size_y, 0, C, 1, 1);
        xrAPI.UIRender->PushPoint(cx - size_x, cy - size_y, 0, C, 0, 0);
        xrAPI.UIRender->PushPoint(cx + size_x, cy - size_y, 0, C, 1, 0);
