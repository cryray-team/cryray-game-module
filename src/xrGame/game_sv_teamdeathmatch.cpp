...
void game_sv_TeamDeathmatch::OnDetachItem(CSE_ActorMP* actor, CSE_Abstract* item)
{
    ...
    if (item->m_tClassID == CLSID_OBJECT_PLAYERS_BAG)
    {
        ...
        /*std::for_each(to_destroy.begin(), to_destroy.end(),
            std::bind(std::mem_fun<void, game_sv_mp, CSE_Abstract*>(&game_sv_mp::DestroyGameItem), this));

        std::for_each(to_reject.begin(), to_reject.end(),
            std::bind1(std::mem_fun<void, game_sv_mp, CSE_Abstract*>(&game_sv_mp::RejectGameItem), this));*/
    };
}
