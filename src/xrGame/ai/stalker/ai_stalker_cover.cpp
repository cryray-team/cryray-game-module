...
{
    minimum_enemy_distance = CryRayStaticValues::MIN_SUITABLE_ENEMY_DISTANCE;
...
{
    m_ce_best->setup(position_to_cover_from, CryRayStaticValues::MIN_SUITABLE_ENEMY_DISTANCE, 170.f,
        CryRayStaticValues::MIN_SUITABLE_ENEMY_DISTANCE);
...
void CAI_Stalker::update_best_cover_actuality(const Fvector& position_to_cover_from)
{
    ...
    if (m_best_cover->position().distance_to_sqr(position_to_cover_from) <
        _sqr(CryRayStaticValues::MIN_SUITABLE_ENEMY_DISTANCE))
    {
        m_best_cover_actual = false;
#if 0 // def _DEBUG
