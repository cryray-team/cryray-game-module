...
#include "../xrGameAPI/AI/ai_monsters_misc.h"
#include "../../weapon.h"
#include "../../hit.h"
...
void CAI_Stalker::Die(CObject* who)
{
    ...
    if (m_death_sound_enabled)
    {
        ...
        else if (!m_lastHittedInHead) // dont play sound when head shoted
...
void CAI_Stalker::UpdateCamera()
{
    ...
    {
        update_range_fov(
            new_range, new_fov, memory().visual().current_state2().m_max_view_distance * eye_range, eye_fov);
        if (weapon_shot_effector().IsActive())
