...
float CActor::GetWeaponAccuracy() const
{
    ...
    if (g_State(state))
    {
        ...
        dispersion *= (1.0f +
            (state.fAVelocity / VEL_A_MAX) * m_fDispVelFactor * GetWeaponParam(W, Get_PDM_Vel_F(), 1.0f) *
        ...
        {
            dispersion *= (1.0f +
                m_fDispAccelFactor * GetWeaponParam(W, Get_PDM_Accel_F(), 1.0f) *
    ...
    else if (pWeapon && pWeapon->HudItemData() && !dynamic_cast<CWeaponKnife*>(pWeapon))
    {
        ...
        if (cam_freelook != eflDisabled)
        {
            ...
            dir.setHP(-angle_normalize_signed(old_torso_yaw),
                pitch > 0.f ?
                    ((pWeapon->GetState() == CWeapon::eFire || cam_freelook == eflDisabling) ? pitch : pitch * .6f) :
                    pitch * .8f);
            fire_dir = dir;
        }
        else if ((psActorFlags.test(AF_FIREPOS) || (mstate_real & mcAnyMove)) &&
            (pWeapon->GetZRotatingFactor() != 1.f /*|| dist < 1.f*/))
        {
            // correct barrel direction
            fire_dir = fire_mat.k; // pWeapon->get_lastFD() doesn't seem to work, returns (0,0,1) for all weapons except
                                   // pistols/shotguns
            ...
            pos.add(offs);
            fire_pos = pos;
            const Fmatrix& fire_mat2 =
            pWeapon->HudItemData()->m_model->LL_GetTransform(pWeapon->HudItemData()->m_measures.m_fire_bone); Fvector
            offs = g_player_hud->m_adjust_mode ? g_player_hud->m_adjust_firepoint_shell[0][0] :
            pWeapon->HudItemData()->m_measures.m_fire_point_offset; offs.z -= 1.f; fire_mat2.transform_tiny(fire_pos,
            offs); pWeapon->HudItemData()->m_item_transform.transform_tiny(fire_pos);
