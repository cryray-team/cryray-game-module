...
BOOL CCar::net_Spawn(CSE_Abstract* DC)
{
    ...
    CSE_ALifeCar* car = dynamic_cast<CSE_ALifeCar*>(e);
    ...
    DamageItem::CDamagableItem::RestoreEffect();
...
void CCar::SpawnInitPhysics(CSE_Abstract* D)
{
    ...
    K->CalculateBones(TRUE);
    Init();
    // inits m_driving_wheels,m_steering_wheels,m_breaking_wheels values using recieved in ParceDefinitions & from
    // bone_map PPhysicsShell()->add_ObjectContactCallback(ActorObstacleCallback);
...
void CCar::Hit(SHit* pHDS)
{
    ...
    DamageItem::CDamagableItem::HitEffect();
...
{
    DamageItem::CDamagableItem::ApplyDamage(level);
    switch (level)
    {
...
void CCar::Init()
{
    ...
    DamageItem::CDamagableItem::Init(GetfHealth(), 3);
    float l_time_to_explosion = READ_IF_EXISTS(ini, r_float, "car_definition", "time_to_explosion", 120.f);
    CDelayedActionFuse::Initialize(l_time_to_explosion, DamageItem::CDamagableItem::DamageLevelToHealth(2));
    {
        xr_map<u16, SWheel>::iterator i, e;
        i = m_wheels_map.begin();
        e = m_wheels_map.end();
...
void CCar::OnEvent(NET_Packet& P, u16 type)
{
    ...
    case GE_OWNERSHIP_REJECT: {
        ...
        GetInventory()->DropItem(dynamic_cast<CGameObject*>(O), just_before_destroy, just_before_destroy);
...
{
    CDelayedActionFuse::Initialize(float(et) / 1000.f, DamageItem::CDamagableItem::DamageLevelToHealth(2));
}
