...
ScriptWallmarksArray::ScriptWallmarksArray(LPCSTR section)
{
    ...
    R_ASSERT2(
        pSettings->section_exist(section), make_string("[ScriptWallmarksManager] Can't find section '%s'", m_section));
    LPCSTR wallmarks_string = READ_IF_EXISTS(pSettings, r_string, m_section, "wallmarks", nullptr);
    R_ASSERT2(
        wallmarks_string, make_string("[ScriptWallmarksManager] Can't find 'wallmarks' in section '%s'", m_section));
    ...
    LPCSTR section, CScriptGameObject* ignore_obj, float ttl)
{
    ...
    BOOL reach_wall = Level().ObjectSpace.RayPick(start_pos, dir, trace_dist, collide::rqtBoth, result,
    ...
    if (reach_wall)
    {
        ...
        if (!pMaterial->Flags.is(SGameMtl::flSuppressWallmarks))
        {
            ...
            xrAPI.Render->add_StaticWallmark(FindSection(section), end_point, wallmark_size, pTri, pVerts, ttl, true);
        }
    }
}

void ScriptWallmarksManager::PlaceSkeletonWallmark(
    CScriptGameObject* obj, LPCSTR section, Fvector start, Fvector dir, float size, float ttl)
{
    if (!obj)
    {
    ...
    xrAPI.Render->add_SkeletonWallmark(&object->renderable.xform, object->Visual()->dcast_PKinematics(), FindSection(section),
        start, dir, size, ttl, true);
}
