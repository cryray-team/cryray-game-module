    ...
    if (reach_wall)
    {
        ...
        if (pMaterial->Flags.is(SGameMtl::flBloodmark))
        {
            ...
            VERIFY(!pwallmarks_vector->empty());
            {
                ...
                xrAPI.Render->add_StaticWallmark(pwallmarks_vector, end_point, wallmark_size, pTri, pVerts);
            }
        }
    }
}
...
{
    IC bool operator()(CCF_Skeleton::SElement const& element, u16 element_id) const
    {
...
{
    IC bool operator()(std::pair<u16, float> const& left, std::pair<u16, float> const& right) const
    {
...
void CEntityAlive::fill_hit_bone_surface_areas() const
{
    ...
    m_hit_bone_surface_areas.xr_clear();
