...
void CSightManager::Exec_Look(float time_delta)
{
    ...
    const float g_ai_aim_min_speed = pStructCryRay->hard_vis_npc ? ai_aim_min_speed_hard : ai_aim_min_speed_medium;
    const float g_ai_aim_min_angle = pStructCryRay->hard_vis_npc ? ai_aim_min_angle_hard : ai_aim_min_angle_medium;
    const float g_ai_aim_max_angle = pStructCryRay->hard_vis_npc ? ai_aim_max_angle_hard : ai_aim_max_angle_medium;
...
static IC float lerp_sight(float low, float high, float value)
{
    float result;
    if (low > high)
    {
...
void CSightManager::process_action(float const time_delta)
{
    ...
    m_current.m_shoulder.m_factor =
        lerp_sight(m_current.m_shoulder.m_factor, factors.y, s_factor_lerp_speed * time_delta);
    VERIFY(_valid(m_current.m_shoulder.m_factor));
