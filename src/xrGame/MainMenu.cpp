...
void CMainMenu::Activate(bool bActivate)
{
    ...
    if (bActivate && CryRayFlags32.test(RS_DISCORD_RPC_ACTIVE_ANOMALY))
        discord_sdk::updateDiscordPresence();
    ...
    if (bActivate)
    {
        ...
        pAutosaveManager->SetbTimerAutosaveStart(false);

        m_Flags.set(flRestoreConsole, Console->isOpen());
...
void CMainMenu::IR_OnKeyboardPress(int dik)
{
    ...
    {
        const bool IsConsole = Console->isOpen();
        xrAPI.Render->Screenshot(IsConsole);
        return;
    }
...
void CMainMenu::OnFrame()
{
    ...
    if (m_Flags.test(flGameSaveScreenshot) && Device.dwFrame > m_screenshotFrame)
    {
        ...
        const bool IsConsole = Console->isOpen();
        xrAPI.Render->Screenshot(IsConsole, IRender_interface::SM_FOR_GAMESAVE, m_screenshot_name);
...
void CMainMenu::Screenshot(IRender_interface::ScreenshotMode mode, LPCSTR name)
{
    ...
    {
        const bool IsConsole = Console->isOpen();
        xrAPI.Render->Screenshot(IsConsole, mode, name);
    }
    else
    {
