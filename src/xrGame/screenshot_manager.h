...
public:
    ...
    using complete_callback_t = fastdelegate::FastDelegate<void(u8 const*, u32, u32)>;
...
private:
    ...
    IC bool is_making_screenshot() const { return !!(m_state & making_screenshot); };
    IC bool is_drawing_downloads() const { return !!(m_state & drawing_download_states); };
    IC bool is_active() const { return (is_making_screenshot() || is_drawing_downloads()); };
    ...
    void timer_end();
#else
    IC void timer_begin(LPCSTR comment) {}
