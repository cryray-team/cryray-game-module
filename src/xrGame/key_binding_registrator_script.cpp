...
{
    module(L)[def("dik_to_bind", &dik_to_bind), def("bind_to_dik", &get_action_dik), def("key_state", &key_state),
        ...
        class_<enum_exporter<EGameActions>>("key_bindings")
            .enum_("commands")[value("kFWD", int(kFWD)), value("kARTEFACT", int(kARTEFACT)), value("kBACK", int(kBACK)),
                ...
                value("kCUSTOM15", int(kCUSTOM15)), value("kCUSTOM16", int(kCUSTOM16)),
                value("kCUSTOM17", int(kCUSTOM17)), value("kCUSTOM18", int(kCUSTOM18)),
                value("kCUSTOM19", int(kCUSTOM19)), value("kCUSTOM20", int(kCUSTOM20)),
                value("kCUSTOM21", int(kCUSTOM21)), value("kCUSTOM22", int(kCUSTOM22)),
                value("kCUSTOM23", int(kCUSTOM23)), value("kCUSTOM24", int(kCUSTOM24)),
                value("kCUSTOM25", int(kCUSTOM25)), value("kTORCH", int(kTORCH)),
                ...
                value("kWPN_FUNC", int(kWPN_FUNC)), value("kWPN_FIREMODE_PREV", int(kWPN_FIREMODE_PREV)), // Tronex:
                                                                                                          // added
                value("kWPN_FIREMODE_NEXT", int(kWPN_FIREMODE_NEXT)), // Tronex: added
                value("kUSE", int(kUSE)), value("kDROP", int(kDROP)), value("kSCORES", int(kSCORES)),
                value("kCHAT", int(kCHAT)), value("kSCREENSHOT", int(kSCREENSHOT)), value("kQUIT", int(kQUIT)),
                value("kCONSOLE", int(kCONSOLE)), value("kINVENTORY", int(kINVENTORY)), value("kBUY", int(kBUY)),
                value("kSKIN", int(kSKIN)), value("kTEAM", int(kTEAM)), value("kSAFEMODE", int(kSAFEMODE)),
        ...
        class_<key_binding_registrator>("DIK_keys")
            .enum_("dik_keys")[value("DIK_ESCAPE", int(DIK_ESCAPE)), value("DIK_2", int(DIK_2)),
                ...
                value("DIK_RETURN", int(DIK_RETURN)), value("DIK_NUMPADENTER", int(DIK_NUMPADENTER))
