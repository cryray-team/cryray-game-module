...
dynamic_cast<CSE_ALifeItemWeapon*>(ai().alife().objects().object(*I)); if (!l_tpALifeItemWeapon) continue;
...
dynamic_cast<CSE_ALifeItemAmmo*>(ai().alife().objects().object(children[i])); if (!l_tpALifeItemAmmo) continue;
...
dynamic_cast<CSE_ALifeItemAmmo*>(ai().alife().objects().object(children[j])); if (!l_tpALifeItemAmmo1) {
...
= l_tpALifeItemAmmo1;
    ...
    for (int i=0, j=0; i<n; ++i,++j) {
        ...
        CSE_ALifeItemAmmo		*l_tpALifeItemAmmo =
dynamic_cast<CSE_ALifeItemAmmo*>(ai().alife().objects().object(children[i])); if (!l_tpALifeItemAmmo ||
...
dynamic_cast<CSE_ALifeInventoryItem*>(ai().alife().objects().object(*children.begin())); R_ASSERT2
...
dynamic_cast<CSE_ALifeHumanAbstract*>(ai().alife().objects().object(l_tpALifeGroupAbstract->m_tpMembers[0])); R_ASSERT
...
l_dwCurrentValue = iFloor(ai().ef_storage().m_pfDetectorType->ffGetValue()+.5f); if (l_dwCurrentValue > l_dwBestValue) {
                ...
                m_tpBestDetector	= const_cast<CSE_ALifeDynamicObject*>(dynamic_cast<const
CSE_ALifeDynamicObject*>(ai().ef_storage().alife().member_item()));
    ...
    for ( ; I != E; ++I) {
        ...
        CSE_ALifeInventoryItem		*l_tpALifeInventoryItem =
dynamic_cast<CSE_ALifeInventoryItem*>(l_tpALifeDynamicObject);
...
//		if (!tpObjectVector) {
//			alife().graph().attach
(*this,l_tpALifeItemBest,dynamic_cast<CSE_ALifeDynamicObject*>(l_tpALifeItemBest)->m_tGraphID);
...
(std::find(tpObjectVector->begin(),tpObjectVector->end(),(*I)->base()->ID) == tpObjectVector->end()))) {
    ...
    if (l_tpALifeItemBest) {
        ...
        if (!tpObjectVector)
            alife().graph().attach
(*this,l_tpALifeItemBest,dynamic_cast<CSE_ALifeDynamicObject*>(l_tpALifeItemBest)->m_tGraphID); else children.push_back
(l_tpALifeItemBest->base()->ID);

        m_dwTotalMoney			-= l_tpALifeItemBest->m_dwCost;
        attach_available_ammo
(dynamic_cast<CSE_ALifeItemWeapon*>(l_tpALifeItemBest),alife().m_temp_item_vector,tpObjectVector); m_dwTotalMoney =
l_dwSafeMoney;
...
(*this,*I,dynamic_cast<CSE_ALifeDynamicObject*>(*I)->m_tGraphID); else { children.push_back((*I)->base()->ID);
            }
            ++l_dwCount;
