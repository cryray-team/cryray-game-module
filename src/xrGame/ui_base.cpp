...
void ui_core::PopScissor()
{
    ...
    if (m_Scissors.empty())
        xrAPI.UIRender->SetScissor(NULL);
    else
    {
...
void ui_core::pp_start()
{
    ...
    m_pp_scale_.set(float(xrAPI.Render->getTarget()->get_width()) / float(UI_BASE_WIDTH),
        float(xrAPI.Render->getTarget()->get_height()) / float(UI_BASE_HEIGHT));
    m_2DFrustumPP.CreateFromRect(Frect().set(
        0.0f, 0.0f, float(xrAPI.Render->getTarget()->get_width()), float(xrAPI.Render->getTarget()->get_height())));

    m_current_scale = &m_pp_scale_;

    g_current_font_scale.set(float(xrAPI.Render->getTarget()->get_width()) / float(Device.dwWidth),
        float(xrAPI.Render->getTarget()->get_height()) / float(Device.dwHeight));
}
