...
public:
    ...
    static bool isBool(const char* str)   { return xr_strcmp(str, "on") == 0 || xr_strcmp(str, "yes") == 0 || xr_strcmp(str, "true") == 0 || xr_strcmp(str, "1") == 0; }
private:
    string_path m_file_name;
    Root DATA;

    void Load(IReader* F, LPCSTR path, allow_include_func_t allow_include_func = NULL);
public:
    ...
    bool r_bool_m(const char* S, const char* L) const;
    bool r_bool_m(const shared_str& S, const char* L) const { return r_bool_m(*S, L); }
...
extern XRCORE_API CInifile const* pCryRaySettings;
