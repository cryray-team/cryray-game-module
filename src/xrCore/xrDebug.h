...
private:
	static std::vector<std::string> BuildStackTrace(PCONTEXT threadCtx, u16 maxFramesCount);
	static bool symEngineInitialized;
	static Lock dbgHelpLock;
	static bool GetNextStackFrameString(LPSTACKFRAME stackFrame, PCONTEXT threadCtx, std::string& frameStr);
	static bool InitializeSymbolEngine();
	static void DeinitializeSymbolEngine(void);
	static LONG WINAPI UnhandledFilter(_EXCEPTION_POINTERS* exPtrs);

public:
	static LPCSTR ErrorToString(long code);
