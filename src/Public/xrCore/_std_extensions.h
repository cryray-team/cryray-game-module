...
#ifndef MASTER_GOLD
IC errno_t xr_strcpy(LPSTR destination, size_t const destination_size, const char* source)
{
    return strcpy_s(destination, destination_size, source);
}

IC errno_t xr_strcat(LPSTR destination, size_t const buffer_size, const char* source)
{
    return strcat_s(destination, buffer_size, source);
}

IC int __cdecl xr_sprintf(LPSTR destination, size_t const buffer_size, const char* format_string, ...)
{
    va_list args;
...
template <int count>
IC int __cdecl xr_sprintf(char (&destination)[count], const char* format_string, ...)
{
    va_list args;
    va_start(args, format_string);
...
template <int count>
IC errno_t xr_strcpy(char (&destination)[count], const char* source)
{
    return xr_strcpy(destination, count, source);
}

template <int count>
IC errno_t xr_strcat(char (&destination)[count], const char* source)
{
...
IC char* xr_strconcat(StrType& dest, const StrType2& arg1, const Args&... args) {
