...
template <class T>
IC constexpr T _lerp_vector(const T& _val_a, const T& _val_b, const float& _factor)
{
    return (_val_a * (1.0 - _factor)) + (_val_b * _factor);
}

template <class T>
IC constexpr T _lerpc_vector(const T& _val_a, const T& _val_b, const float& _factor)
{
...
// inertion
IC float _inertion_vector(float _val_cur, float _val_trgt, float _friction)
{
