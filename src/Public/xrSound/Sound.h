...
public:
    ...
    IC virtual void update_velocity(const float dt) { velocity.set(accVelocity).div(dt); }
...
{
    xrAPI.Sound->_create_data(*this, fName, sound_type, game_type);
}
IC ref_sound_data::~ref_sound_data() { xrAPI.Sound->_destroy_data(*this); }

IC void ref_sound::create(LPCSTR name, esound_type sound_type, int game_type)
{
    VERIFY(!xrAPI.Sound->i_locked());
    xrAPI.Sound->create(*this, name, sound_type, game_type);
}
IC void ref_sound::attach_tail(LPCSTR name)
{
    VERIFY(!xrAPI.Sound->i_locked());
    xrAPI.Sound->attach_tail(*this, name);
}

IC void ref_sound::clone(const ref_sound& from, esound_type sound_type, int game_type)
{
    VERIFY(!xrAPI.Sound->i_locked());
    xrAPI.Sound->clone(*this, from, sound_type, game_type);
}
IC void ref_sound::destroy()
{
    VERIFY(!xrAPI.Sound->i_locked());
    xrAPI.Sound->destroy(*this);
}
IC void ref_sound::play(CObject* O, u32 flags, float d)
{
    VERIFY(!xrAPI.Sound->i_locked());
    xrAPI.Sound->play(*this, O, flags, d);
}
IC void ref_sound::play_at_pos(CObject* O, const Fvector& pos, u32 flags, float d)
{
    VERIFY(!xrAPI.Sound->i_locked());
    xrAPI.Sound->play_at_pos(*this, O, pos, flags, d);
}
    ...
    CObject* O, u32 flags, float d, Fvector* pos, float* vol, float* freq, Fvector2* range)
{
    ...
    xrAPI.Sound->play_no_feedback(*this, O, flags, d, pos, vol, freq, range);
}
...
IC void ref_sound::set_time(float time)
{
    VERIFY(!xrAPI.Sound->i_locked());
    if (_feedback())
        _feedback()->set_time(time);
}
