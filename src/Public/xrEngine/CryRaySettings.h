...
namespace
{
    ...
    void InitCryRayConfigSettings()
    {
        ...
        auto l_OtherSettings = []
        {
            ...
            LPCSTR OldLoadingScreen = pCryRaySettings->r_string("render_cryray", "old_load_screen");

            if (xr_strcmp("false", OldLoadingScreen) == 0)
            {
                pSGameSettings->bOldLoadScreen = false;
            }
            else
            {
                pSGameSettings->bOldLoadScreen = true;
            }
        };
