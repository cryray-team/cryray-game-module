...
inline LPSTR sz_cpy(LPSTR destination, const u32& size, LPCSTR source)
{
...
template <int size>
inline LPSTR sz_cpy(char (&destination)[size], LPCSTR source)
{
    xr_strcpy(destination, size, source);
    return (destination);
}

inline LPSTR sz_cat(LPSTR destination, const u32& size, LPCSTR source)
{
...
template <int size>
inline LPSTR sz_cat(char (&destination)[size], LPCSTR source)
{
    xr_strcat(destination, size, source);
    return (destination);
}

inline u32 sz_len(LPCSTR string) { return ((u32)xr_strlen(string)); }
    ...
    luabind::detail::object_rep* object, lua_State* state)
{
    ...
    for (i = 0; lua_next(state, -2); ++i)
    {
        ...
        LPCSTR name = lua_to_string(state, -2);
