...
CSE_ALifeItemTorch::CSE_ALifeItemTorch(LPCSTR caSection) : CSE_ALifeItem(caSection)
{
    ...
    m_attached = false;
    m_battery_state = READ_IF_EXISTS(pSettings, r_u16, caSection, "battery_duration", 70000);
}
...
void CSE_ALifeItemWeapon::STATE_Read(NET_Packet& tNetPacket, u16 size)
{
    ...
    if (a_current_addon.data == 90) // Alun: Hack because all.spawn ammo_current all set to 90. Since this unlikely
                                    // anyway (launcher=26, silencer=1, scope=0), this hack is okay.
...
u16 CSE_ALifeItemWeapon::get_ammo_total() { return ((u16)90); }
