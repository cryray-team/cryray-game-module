...
bool r_eof_script(IReader* self) { return (!!self->eof()); }

LPCSTR r_stringZ_script(IReader* self)
{
    shared_str temp;
    self->r_stringZ(temp);
    return (*temp);
...
bool r_bool_script(IReader* self) { return (!!self->r_u8()); }

void r_fvector3_script(IReader* self, Fvector* arg0) { self->r_fvector3(*arg0); }
