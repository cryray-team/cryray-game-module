...
public:
    ...
    virtual void Execute(LPCSTR args) { strncpy_s(value, size, !xr_strcmp(args, "(NULL)") ? "" : args, size - 1); }
...
public:
    ...
    virtual void Info(TInfo& I)
    {
        xr_sprintf(I, sizeof(I), "ivector4 in range [%i,%i,%i,%i]-[%i,%i,%i,%i]", min.x, min.y, min.z, min.w, max.x,
            max.y, max.z, max.w);
    }
    ...
    {
        TStatus str;
        xr_sprintf(str, sizeof(str), "(%i, %i, %i, %i) (current) [(%i,%i,%i,%i)-(%i,%i,%i,%i)]", value->x, value->y,
            value->z, value->w, min.x, min.y, min.z, min.w, max.x, max.y, max.z, max.w);
        tips.push_back(str);
        IConsole_Command::fill_tips(tips, mode);
    }
