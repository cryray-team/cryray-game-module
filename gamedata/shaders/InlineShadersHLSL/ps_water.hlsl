...
float4 main( vf I ) : SV_Target
{
	...
	// Normal Textures
	float3	n0	= s_nmap0.Sample(smp_base, I.tnorm0.xy).rgb;
	float3	n1	= s_nmap0.Sample(smp_base, I.tnorm0.zw).rgb;
	...
	float2 N_Scr = normalize(float3(basenormal.x * G_SSR_WATER_REFRACTION, basenormal.y * G_SSR_WATER_REFRACTION, basenormal.z)).xy;
	...
	float3 Nv = normalize(mul(m_V, float4(Nw, 0.0f)).xyz);
	...
	float blue_noise = s_bluenoise.Sample(smp_linear, I.tbase.xy * float2(0.05f, 0.05f) + NN.xy).b * 1.5f;
	...
	float2 N_Tex = normalize(float3(basenormal.x * G_SSR_WATER_TEX_DISTORTION, basenormal.y * G_SSR_WATER_TEX_DISTORTION, basenormal.z)).xy;

	// Get Position with Refraction discard
	float3 _P2 = SSFX_get_position(PosTc + Refraction_UV, 0.f).xyz;
	...
	float3 w_s = mul(m_inv_V, float4(Pv, 0.0f) ).xyz;

	// Caustics offset anim
	float4 CausticsT = float4( timers.x * 0.1f, 0.f, -timers.x * 0.07f, 0.2f );

	// Fake Caustics
	float3 Ca0 = saturate(s_watercaustics.Sample(smp_base, w_b.xz * (0.19f / G_SSR_WATER_CAUSTICS_SCALE) + CausticsT.xy + N_Scr ).xyz - float3(0.1f, 0.1f, 0.1f));
	float3 Ca1 = saturate(s_watercaustics.Sample(smp_base, w_b.xz * (0.11f / G_SSR_WATER_CAUSTICS_SCALE) + CausticsT.zw + N_Scr ).xyz - float3(0.1f, 0.1f, 0.1f));
	...
	acc = lerp(fog_color.xyz, acc, fogging);
	
	float height_fog = get_height_fog_water(_P.xyz, I.p_world.xyz);
	acc = lerp(acc, fog_color.xyz, height_fog);
	...
	acc		= lerp(acc, leaves.rgb, leaves.a*fLeavesFactor);
