...
#include "DX10Fog\fluid_common_tricubic.h"
...
//		return;
	...
	{   
		...
		float3 interpColor = fireTransferFunction.SampleLevel(samLinearClamp,float2(lookUpVal,0.f),0).rgb; 
...
//	   return float4 (1,0,0,saturate(rayData.w/5));
    ...
    float3 stepVec = normalize( (rayOrigin.xyz - eyeOnGrid.xyz) * gridDim.xyz ) * recGridDim.xyz * 0.5;
